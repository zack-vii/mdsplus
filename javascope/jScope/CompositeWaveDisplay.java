...
final public class CompositeWaveDisplay extends JApplet implements WaveContainerListener{
    ...
    public class AppendThread extends Thread{
            ...
            while(true){
                try{
                    ...
                    for(int i = 0; i < signals2DVector.size(); i++){
                        signals2DVector.elementAt(i).setMode2D(Signal.MODE_PROFILE);
                    }
        ...
        */
        private void processPacket(UpdSignalData usd) {
            switch(usd.type){
                case Signal.TYPE_1D:
                    if(usd.name != null) processsSignal1DPacket(usd);
                    else processsSignals1DPacket(usd);
                    break;
                case Signal.TYPE_2D:
                    if(usd.name != null) processsSignal2DPacket(usd);
                    else processsSignals2DPacket(usd);
                    break;
            }
        }
    ...
    public static void addProtocol(DataAccess dataAccess) {
    ...
    public void init() {
        ...
        WavePopup wave_popup = new MultiWavePopup(new SetupWaveformParams(frame, "Waveform Params"), new ProfileDialog(null, null));
    ...
    private static boolean translateToBoolean(String value) {
    ...
    private void getSignalsParameter() {
        ...
        */
        ...
        if(param != null) wave_container.setPrintWithLegend(translateToBoolean(param));
        param = getParameter("PRINT_BW");
        if(param != null) wave_container.setPrintBW(translateToBoolean(param));
        try{
    ...
    private static String getParameterValue(String context, String param) {
        String value = null;
    ...
    public void processWaveContainerEvent(WaveContainerEvent e) {
        ...
        switch(event_id){
            case WaveContainerEvent.WAVEFORM_EVENT:
                ...
                if(isApplet) showStatus(s);
                else point_pos.setText(s);
                break;
        }
    }
     ...
     *            Marker point
     */
    public void addSignal(float[] x, float[] y, int row, int column, String color, String label, boolean inter, int marker) {
        addSignal(new Signal(x, y), row, column, color, label, inter, marker);
    }

    public void addSignal(double[] x, float[] y, int row, int column, String color, String label, boolean inter, int marker) {
        addSignal(new Signal(x, y), row, column, color, label, inter, marker);
    }

    public void addSignal(Signal sig, int row, int column, String color, String label, boolean inter, int marker) {
        if(color != null){
    ...
    private static String getExpression(String paramString, boolean infoFlag) throws IOException {
        ...
        Object localObject = DataAccessURL.getDataAccess(str8);
    ...
    public void addFrames(String url, int row, int column) {
        ...
        WaveInterface wi = null;
        DataAccess da = null;
        if(DataAccessURL.getNumProtocols() == 0) setDataAccess();
        c = wave_container.getGridComponent(row, column);
        try{
    ...
    public void addSignal(String url, int row, int column, String color, String label, boolean inter, int marker) {
        Component c = null;
        MultiWaveform w = null;
        WaveInterface wi = null;
        DataAccess da = null;
    ...
    public void removeAllSignals() {
        if(wave_container != null) wave_container.RemoveAllSignals();
     ...
     *            Signal name
     */
    public void addSignal(float[] x, float[] y, int row, int column, String color, String label) {
        addSignal(x, y, row, column, color, label, true, 0);
    }

    public void addSignal(double[] x, float[] y, int row, int column, String color, String label) {
        addSignal(x, y, row, column, color, label, true, 0);
    }
    ...
    public void addSignal(Signal s, int row, int col) {
        ...
        if(c != null){
            ...
            if(automatic_color) s.setColorIdx(w.GetShowSignalCount());
            w.addSignal(s);
        }else{
            w = (MultiWaveform)wave_container.CreateWaveComponent();
            w.addSignal(s);
        ...
        w.Update();
        if(isShowing() && !isValid()) wave_container.update();
    }
