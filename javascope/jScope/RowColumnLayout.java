...
final public class RowColumnLayout implements LayoutManager{
    ...
    public float getPercentHeight(int i) {
        if(i < percent_height.length) return percent_height[i];
        return 0;
    }
    ...
    public float getPercentWidth(int i) {
        if(i < row.length) return percent_width[i];
        return 0;
    }
    ...
    private void setSizes(Container parent) {
        ...
        minWidth = 0;
        minHeight = 0;
        if(2 * b_comp + 1 != nComps){ throw new IllegalArgumentException("Invalid number of component in RowColumnLayout"); }
        percent_height = new float[b_comp + 1];
        ...
        for(int i = 0; i < row.length; i++){// column; i++) {
            ...
            if(totH > preferredHeight) preferredHeight = totH;
            totW += maxW[i];
        }
        preferredWidth = totW;
    ...
    public void DrawResize(Component _b, int x, int y) {
            ...
            for(j = 0; j < row.length && !found; j++){// column && !found; j++) {
                for(i = 0; i < row[j]; i++){
                    ...
                    if(_b == b){
                        col_idx = j;
                        comp_idx = idx;
                        if(i == row[j] - 1) resize_mode = HORIZONTAL;
                        else resize_mode = VERTICAL;
                        found = true;
                        break;
                    }
                    idx++;
        ...
        if(resize_mode == VERTICAL){
            ...
            if(y < 0){
                ...
                for(j = 0, jj = 0, i = b_comp + comp_idx; i > b_comp + k - 1; i--, j++){
                    ...
                    if(curr_height + pos < MIN_SIZE_H * (num_line + 1)) num_line++;
                    if(start_pos + pos + (k - comp_idx - 1) * MIN_SIZE_H <= 0) return;
                    for(n_draw = 0; jj < num_line + 1; jj++)
                        if(Math.abs(pos) + jj * MIN_SIZE_H < curr_height) n_draw++;
                        else break;
                    if(n_draw > 0){
                ...
                for(j = 0, jj = 0, i = b_comp + comp_idx + 1; i < b_comp + k; i++, j++){
                    ...
                    curr_height += r.height;
                    if(curr_height - pos < MIN_SIZE_H * (num_line + 1)) num_line++;
                    if(start_pos + pos + (k - comp_idx - 1) * MIN_SIZE_H > maxHeight) return;
                    for(n_draw = 0; jj < num_line + 1; jj++)
                        if(pos + jj * MIN_SIZE_H < curr_height) n_draw++;
                        else break;
                    if(n_draw > 0){
        ...
        if(resize_mode == HORIZONTAL){
            ...
            if(x < 0){
                pos = r.width + x;
                if(pos < MIN_SIZE_W) pos = MIN_SIZE_W;
            }else{
                pos = x;
                if(r.width - pos < MIN_SIZE_W) pos = r.width - MIN_SIZE_W;
            }
    ...
    public void ResizeRowColumn(Component _b, int x, int y) {
        ...
        for(int i = 0; i < col_idx; i++)
            k += row[i];
        if(resize_mode == VERTICAL){
            if(maxHeight < row[col_idx] * (MIN_SIZE_H + vgap) - vgap) return;
            if(pos_y < y){
                start_comp = k;
                end_comp = k + row[col_idx];
                inc_comp = 1;
                max_y = maxHeight - (row[col_idx] - (comp_idx - k) - 1) * (MIN_SIZE_H + vgap) + vgap;
            }else{
            ...
            for(k = start_comp, new_y = 0; k != end_comp; k += inc_comp){
                ...
                if(k == comp_idx) percent_height[k] = (float)(resize_y - curr_y) / currMaxHeight;
                else if(curr_y + currHeight - new_y < MIN_SIZE_H) percent_height[k] = (float)(MIN_SIZE_H) / currMaxHeight;
                else percent_height[k] = (float)(curr_y + currHeight - new_y) / currMaxHeight;
        ...
        if(resize_mode == HORIZONTAL){
            if(maxWidth < column * (MIN_SIZE_W + hgap) - hgap) return;
            currMaxWidth = (maxWidth - (column - 1) * hgap);
            for(int i = 0; i < col_idx; i++)
                curr_x += currMaxWidth * percent_width[i] + hgap;
            min_x = curr_x + MIN_SIZE_W;
            max_x = (int)(curr_x + currMaxWidth * (percent_width[col_idx] + percent_width[nextColumn(col_idx)])) + hgap;
        ...
        for(int i = 0; i < row.length; i++)// column; i++)
        {
            ...
            for(int j = 0; j < row[i]; j++){
                ...
                if(c.isVisible()){
                    ...
                    d.width = currWidth;
                    // if(i != column - 1)
                    if(curr_col != column - 1) d.width = (int)(currMaxWidth * percent_width[i]);
                    else d.width = maxWidth - x;
                    c.setBounds(x, c.getBounds().y, d.width, c.getBounds().height);
                    if(k < b_comp){
                        Component b = parent.getComponent(k);
                        if(j == row[i] - 1) b.setBounds(x + d.width - hgap - 2, maxHeight - bx_pos, 4, 8);
                        else{
                            int p = by_pos;
                            if(d.width < by_pos + 8) p = d.width - 1;
    ...
    private void ResizeColumn(Container parent, int col_idx) {
        ...
        for(int j = 0; j < row[col_idx]; j++){
            ...
            if(c.isVisible()){
                ...
                if(j != row[col_idx] - 1) d.height = (int)(currMaxHeight * percent_height[k]);
                else d.height = maxHeight - y;
                c.setBounds(c.getBounds().x, y, c.getBounds().width, d.height);
                previousHeight = d.height + vgap;
                if(k < b_comp){
                    Component b = parent.getComponent(k);
                    if(j == row[col_idx] - 1) b.setBounds(c.getBounds().x + c.getBounds().width - hgap - 2, maxHeight - bx_pos, 4, 8);
                    else{
                        int p = by_pos;
                        if(d.width < by_pos + 8) p = d.width - 1;
     ...
     */
    private static void DrawXResize(Component c, int x) {
     ...
     *            component on which draw line/s
     */
    private static void DrawYResize(Component c, int y, int n_line, int space) {
        // if(System.getProperty("java.version").indexOf("1.2") != -1 && !c.isDoubleBuffered())
        // return;
        // else
        if(!(c instanceof Waveform || c instanceof MultiWaveform)) return;
