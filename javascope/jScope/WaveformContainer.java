...
public class WaveformContainer extends RowColumnContainer implements WaveformManager, WaveformListener, Printable{
        ...
        addMouseListener(new MouseAdapter(){
            ...
            public void mousePressed(MouseEvent e) {
                ...
                if(wave_popup != null){
                    ...
                    int tran_x = 0;
                    int tran_y = 0;
                    if(y + p.y + wp_size.height > scr_dim.height) tran_y = y + p.y + wp_size.height - scr_dim.height + 20;
                    if(x + p.x + wp_size.width > scr_dim.width) tran_x = x + p.x + wp_size.width - scr_dim.width + 20;
    ...
    public void processWaveformEvent(WaveformEvent e) {
        ...
        switch(e.getID()){
            ...
            case WaveformEvent.COPY_PASTE:
                if(copy_waveform != null) NotifyChange(w, copy_waveform);
                return;
            ...
            case WaveformEvent.MEASURE_UPDATE:
                if(w.GetMode() == Waveform.MODE_POINT){
                    ...
                    else if(e.is_mb2) AllSameXScaleAutoY(w);
    ...
    public int GetWaveIndex(Waveform w) {
        ...
        if(idx < getGridComponentCount()) return idx;
        return -1;
    }
    ...
    public Waveform GetSelectPanel() {
        ...
        if(sel_wave == null){
            int i;
            for(i = 0; i < getGridComponentCount() && GetWavePanel(i) != null && (GetWavePanel(i).GetShowSignalCount() != 0 || GetWavePanel(i).IsImage()); i++);
    ...
    public void ResetDrawPanel(int _row[]) {
        ...
        Component c[] = null;
        if(num > 0) c = CreateWaveComponents(num);
        update(_row, c);
        if(sel_wave != null) sel_wave.SelectWave();
        // System.gc();
    }
    ...
    public void Deselect() {
        if(sel_wave != null) sel_wave.DeselectWave();
        sel_wave = null;
    }
    ...
    public void SetCopySource(Waveform w) {
        ...
        if(copy_waveform != null) copy_waveform.SetCopySelected(false);
        copy_waveform = w;
    }
    ...
    public void PrintAll(Graphics g, int st_x, int st_y, int height, int width) {
        ...
        for(i = k = 0, px = st_x; i < rows.length; i++){
            ...
            for(j = pos = 0, py = st_y; j < rows[i]; j++){
                ...
                g.translate(0, py);
                if(j == rows[i] - 1 && pos + curr_height != height) curr_height = height - pos;
                g.setClip(0, 0, curr_width, curr_height);
                w = GetWavePanel(k);
                if(w != null){
                    int print_mode = Waveform.PRINT;
                    if(print_with_legend) print_mode |= MultiWaveform.PRINT_LEGEND;
                    if(print_bw) print_mode |= MultiWaveform.PRINT_BW;
    ...
    private static String getFileName(Waveform w) {
        String out;
