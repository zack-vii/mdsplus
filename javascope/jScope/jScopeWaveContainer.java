...
final class jScopeWaveContainer extends WaveformContainer{
            ...
            while(true){
                try{
                    ...
                    try{
                        ...
                        String msg;
                        if(!abort) msg = "All waveforms are up to date < " + (end - start) + " ms >";
                        else msg = " Aborted ";
    ...
    */
    public String GetServerLabel() {
        if(dp == null && server_item != null && server_item.name != null) return "Can't connect to " + server_item.name;
        if(dp == null && server_item == null) return "Not connected";
    ...
    public void processWaveformEvent(WaveformEvent e) {
        ...
        switch(e.getID()){
            case WaveformEvent.END_UPDATE:
                ...
                if(w.wi.isAddSignal()){
                    String er;
                    if(!w.wi.IsSignalAdded()) prev_add_signal = null;
                    if(w.wi.error != null) er = w.wi.error;
                    else er = ((MdsWaveInterface)w.wi).getErrorString(); // this.brief_error);
                ...
                WaveContainerEvent wce = new WaveContainerEvent(this, WaveContainerEvent.END_UPDATE, "Wave column " + p.x + " row " + p.y + " is updated");
                jScopeWaveContainer.this.dispatchWaveContainerEvent(wce);
                break;
        }
    }
    ...
    public void StartPrint(DocPrintJob prnJob, PrintRequestAttributeSet attrs) {
        ...
        }catch(InterruptedException e){}catch(PrinterException e){}catch(Exception e){}
    }
    ...
    public void EvaluateMainShot(String in_shots) throws IOException {
        ...
        synchronized(mainShotLock){
            ...
            if(main_shot_error == null) main_shots = long_data;
    ...
    public void RemoveAllEvents(UpdateEventListener l) throws IOException {
        jScopeMultiWave w;
        if(dp == null) return;
        if(event != null && event.length() != 0) dp.RemoveUpdateEventListener(l, event);
        if(print_event != null && print_event.length() != 0) dp.RemoveUpdateEventListener(l, print_event);
        for(int i = 0, k = 0; i < 4; i++){
    ...
    public synchronized void AddAllEvents(UpdateEventListener l) throws IOException {
        jScopeMultiWave w;
        if(dp == null) return;
        if(event != null && event.length() != 0) dp.AddUpdateEventListener(l, event);
        if(print_event != null && print_event.length() != 0) dp.AddUpdateEventListener(l, print_event);
        for(int i = 0, k = 0; i < 4; i++){
            for(int j = 0; j < getComponentsInColumn(i); j++, k++){
                w = (jScopeMultiWave)getGridComponent(k);
                w.AddEvent();
    ...
    public void InvalidateDefaults() {
        ...
        for(int i = 0, k = 0; i < rows.length; i++)
            for(int j = 0; j < rows[i]; j++, k++){
                w = (jScopeMultiWave)getGridComponent(k);
                if(w != null) ((MdsWaveInterface)w.wi).default_is_update = false;
            }
    }
    ...
    public synchronized void UpdateAllWave() throws Exception {
        ...
        try{
            ...
            for(int i = 0, k = 0; i < 4 && !abort; i++){
                for(int j = 0; j < rows[i]; j++, k++){
                    if(wave_all[k].wi != null && wave_all[k].isWaveformVisible()) ((MdsWaveInterface)wave_all[k].wi).Update();
                }
            }
            ...
            synchronized(mainShotLock){
                if(main_shots != null){
                    for(int l = 0; l < main_shots.length && !abort; l++){
                        for(int i = 0, k = 0; i < 4 && !abort; i++){
                            for(int j = 0; j < rows[i] && !abort; j++, k++){
                                if(wave_all[k].wi != null && wave_all[k].wi.error == null && wave_all[k].isWaveformVisible() && wave_all[k].wi.num_waves != 0 && ((MdsWaveInterface)wave_all[k].wi).UseDefaultShot()){
                                    wce = new WaveContainerEvent(this, WaveContainerEvent.START_UPDATE, "Update signal column " + (i + 1) + " row " + (j + 1) + " main shot " + main_shots[l]);
                                    dispatchWaveContainerEvent(wce);
                                    ((MdsWaveInterface)wave_all[k].wi).EvaluateShot(main_shots[l]);
                                    if(((MdsWaveInterface)wave_all[k].wi).allEvaluated()){
                                        if(wave_all[k].wi != null) wave_all[k].Update(wave_all[k].wi);
                                    }
                                }
                            }
                        }
            ...
            for(int i = 0, k = 0; i < 4; i++){
                for(int j = 0; j < rows[i]; j++, k++){
                    if(wave_all != null && wave_all[k] != null){
                        if(wave_all[k].wi != null && wave_all[k].wi.error == null && wave_all[k].isWaveformVisible() && wave_all[k].wi.num_waves != 0 && !abort){
                            if(((MdsWaveInterface)wave_all[k].wi).allEvaluated()) continue;
                        ...
                        if(wave_all[k].wi != null && wave_all[k].isWaveformVisible()) wave_all[k].Update(wave_all[k].wi);
                    }
                }
            }
            for(int i = 0, k = 0; i < 4; i++){
    ...
    public static DataServerItem DataServerFromClient(DataServerItem dataServerIn) {
        int c = 0;
        boolean found = false;
        String clientMask;
        ...
        try{
            ...
            String f_name = System.getProperty("user.home") + File.separator + "jScope" + File.separator + "jScope_servers.conf";
            ...
            if(dataServerIn != null){
                ...
                if(!checkIpMask(disIpAdddress, clientMask)) return dataServerIn;
            }
    ...
    public void FromFile(Properties pr, final String prompt, int colorMapping[], ColorMapDialog cmd) throws IOException {
        String prop;
        ...
        if(prop != null){
            if(prop.indexOf("_shots") != -1) def_vals.shot_str = prop.substring(prop.indexOf("[") + 1, prop.indexOf("]"));
            else def_vals.shot_str = prop;
        ...
        if(columns != r_columns){
            ...
            if(columns == 4) pw[3] = Math.abs((float)((1000 - pw[2]) / 1000.));
            if(columns >= 3) pw[2] = Math.abs((float)(((pw[2] != 0) ? (pw[2] - pw[1]) : 1000 - pw[1]) / 1000.));
            if(columns >= 2) pw[1] = Math.abs((float)(((pw[1] != 0) ? (pw[1] - pw[0]) : 1000 - pw[0]) / 1000.));
            if(columns >= 1) pw[0] = Math.abs((float)(((pw[0] == 0) ? 1000 : pw[0]) / 1000.));
        }
        UpdateHeight();
    }
    ...
    public void SetDataServer(DataServerItem server_item, UpdateEventListener l) throws Exception {
        ...
        if(browse_sig != null && browse_sig.isShowing()) browse_sig.setVisible(false);
        try{
            ...
            if(dp != null) dp.AddConnectionListener((ConnectionListener)l);
            if(!server_item.class_name.equals("NotConnectedDataProvider")){
                // Check data server connection
                if(dp.GetShots("0") == null) throw(new Exception("Cannot connect to " + server_item.class_name + " data server"));
            }
    ...
    public void AddSignal(String expr, boolean check_prev_signal) {
        if(expr != null && expr.length() != 0) if(!check_prev_signal || (check_prev_signal && (prev_add_signal == null || !prev_add_signal.equals(expr)))){
    ...
    public void AddSignals(String tree, String shot, String x_expr[], String y_expr[], boolean with_error, boolean is_image) {
        ...
        if(sel_wave.wi == null || is_image){
            ...
            if(!with_error) ((MdsWaveInterface)sel_wave.wi).prev_wi = new MdsWaveInterface(sel_wave, dp, def_vals, IsCacheEnabled());
        }else{
            new_wi = new MdsWaveInterface((MdsWaveInterface)sel_wave.wi);
            new_wi.wave = sel_wave;
            if(!with_error) new_wi.prev_wi = (MdsWaveInterface)sel_wave.wi;
            sel_wave.wi = new_wi;
        }
    ...
    public void ToFile(PrintWriter out, String prompt) throws IOException {
        ...
        if(server_item != null){
            ...
            if(server_item.argument != null) WaveInterface.WriteLine(out, prompt + "data_server_argument: ", server_item.argument);
            if(server_item.user != null) WaveInterface.WriteLine(out, prompt + "data_server_user: ", server_item.user);
            if(server_item.browse_class != null) WaveInterface.WriteLine(out, prompt + "data_server_browse_class: ", server_item.browse_class);
            if(server_item.browse_url != null) WaveInterface.WriteLine(out, prompt + "data_server_browse_url: ", server_item.browse_url);
            if(server_item.tunnel_port != null) WaveInterface.WriteLine(out, prompt + "data_server_tunnel_port: ", server_item.tunnel_port);
            WaveInterface.WriteLine(out, prompt + "fast_network_access: ", "" + server_item.fast_network_access);
            if(server_item.enable_cache) WaveInterface.WriteLine(out, prompt + "enable_cache: ", "" + server_item.enable_cache);
        }
        ...
        out.println();
        for(int i = 1, pos = 0; i < getColumns(); i++){ // , k = 0
            // w = (jScopeMultiWave)getGridComponent(k);
            // wi = (MdsWaveInterface)w.wi;
    ...
    public synchronized void Refresh(jScopeMultiWave w, final String label) {
        Point p = null;
        if(add_sig) p = getSplitPosition();
        if(p == null) p = getComponentPosition(w);
        ...
        */
        if(def_vals != null && def_vals.isSet()) this.SetCacheState(false);
        WaveContainerEvent wce = new WaveContainerEvent(this, WaveContainerEvent.START_UPDATE, label + " wave column " + p.x + " row " + p.y);
        jScopeWaveContainer.this.dispatchWaveContainerEvent(wce);
    ...
    public void SaveAsText(jScopeMultiWave w, boolean all) {
        ...
        if(save_as_txt_directory != null && save_as_txt_directory.trim().length() != 0) file_diag.setCurrentDirectory(new File(save_as_txt_directory));
