...
final class Frames extends Canvas{
    ...
    Frames(Frames frames){
        ...
        if(frame_time.size() != 0) frame_time.removeAllElements();
        if(frames.zoom_rect != null) zoom_rect = new Rectangle(frames.zoom_rect);
        if(frames.view_rect != null) view_rect = new Rectangle(frames.view_rect);
    ...
    class FrameCache{
        ...
        void loadFrame(int idx) throws Exception {
            ...
            switch(frameType){
                ...
                case FrameData.BITMAP_IMAGE_32:{
                    ...
                    if(DEBUG.A) DEBUG.printIntArray(buf_out, 1, frameDim.width, frameDim.height, 1);
        ...
        Image getImageAt(int idx) throws IOException {
            ...
            Image img;
            if(frameType == FrameData.BITMAP_IMAGE_32) img = fDesc.image;
            else{
        ...
        private void doBitShift(BufferedImage bi, byte[] buffer) throws Exception {
            ...
            if(nPixels != frameDim.width * frameDim.height) throw new Exception("INTERNAL ERRROR: Inconsistend frame dimension when getting frame");
            int val;
            int rot32 = (-bitShift) % 32;
            for(int j = 0; j < nPixels; j++){
                val = Integer.rotateRight(din.readShort(), rot32) & 0xFFFF;
                if(bitClip) db.setElem(j, val > 255 ? 255 : val);
    ...
    public void setBitShift(int bitShift, boolean bitClip) {
        cache.setBitShift(bitShift, bitClip);
    }
    ...
    public void FlipFrame(byte buf[], Dimension d, int num_byte_pixel) {
        ...
        if(!vertical_flip && !horizontal_flip) return;
    ...
    public int getPixel(int idx, int x, int y) {
        ...
        if(!isInImage(idx, x, y)) return -1;
        // curr_grab_frame = idx;
        byte[] imgBuf = cache.getBufferAt(idx);
        Point p = getImageBufferPoint(x, y);
        if(imgBuf != null) return imgBuf[(p.y * img_width) + p.x];
        return -1;
    }
    ...
    public int[] getPixelsLine(int st_x, int st_y, int end_x, int end_y) {
        ...
        for(int i = 0; i < n_point; i++){
            int x = (int)(st_x + (double)i * (end_x - st_x) / n_point);
            int y = (int)(st_y + (double)i * (end_y - st_y) / n_point);
    ...
    public float[] getFramesTime() {
        ...
        if(frame_time == null || frame_time.size() == 0) return null;
        if(ft == null){
    ...
    public Point getFramePoint(Dimension d) {
        ...
        if(sel_point != null) return getImagePoint(new Point(sel_point.x, sel_point.y), d);
    ...
    public boolean contain(Point p, Dimension d) {
        if(DEBUG.M) System.out.println("Frames.contain(" + sel_point + ", " + d + ")");
        Dimension fr_dim = getFrameSize(curr_frame_idx, d);
    ...
    public int GetFrameIdxAtTime(float t) {
        ...
        if(numFrames == 1) dt = 1;
        else dt = frame_time.elementAt(1).floatValue() - frame_time.elementAt(0).floatValue();
        if(t >= frame_time.elementAt(numFrames - 1).floatValue() + dt) return -1;
        if(t >= frame_time.elementAt(numFrames - 1).floatValue()) return numFrames - 1;
    ...
    public void SetZoomRegion(int idx, Dimension d, Rectangle r) {
        ...
        if(zoom_rect.width == 0) zoom_rect.width = 1;
        if(zoom_rect.height == 0) zoom_rect.height = 1;
        curr_frame_idx = idx;
    }
