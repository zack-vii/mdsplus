...
public class JiNcVar extends JiVarImpl{
    ...
    public JiNcVar(JiNcSource s, String name, JiDim[] dims, long offset, int netCDFtype, boolean isRecord) throws IOException{
        super(s, name);
        mIsRecord = isRecord;
        mDims = dims;
        mNcType = netCDFtype;
    ...
    public JiVar getAtt(String name) throws IOException {
        for(int i = 0; i < mAtts.length; ++i)
            if(name.equals(mAtts[i].getName())) return mAtts[i];
        return null;
    }
    ...
    public int size() {
        ...
        if(mIterator == null) mIterator = new JiSlabIterator((JiNcSource)mSource, this, mDims);
    ...
    public void validateDims(JiDim[] dims) throws IOException {
        if(dims.length != mDims.length) throw new IOException("Dimension lengths don't match");
        for(int i = 0; i < dims.length; ++i){
            ...
            if(cdim.mStart < mdim.mStart || cdim.mStart >= mend || cend < mdim.mStart || cend > mend){
                String message = "Dimensions " + cdim + " " + mdim + " don't match";
