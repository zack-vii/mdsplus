...
final class MdsWaveInterface extends WaveInterface{
    ...
    static final int    B_shot            = 8, B_x_min = 12, B_x_max = 13, B_y_min = 14, B_y_max = 15, B_title = 16, B_x_label = 10, B_y_label = 11, B_exp = 7, B_event = 17, B_default_node = 9, B_update = 0;
    ...
    public MdsWaveInterface(MdsWaveInterface wi){
        ...
        if(wi.in_shot == null || wi.in_shot.trim().length() == 0) shots = wi.shots = null;
        else shots = new long[num_waves];
        for(int i = 0; i < num_waves; i++){
            if(wi.in_label[i] != null) in_label[i] = new String(wi.in_label[i]);
            else in_label[i] = null;
            if(wi.in_x[i] != null) in_x[i] = new String(wi.in_x[i]);
            else in_x[i] = null;
            if(wi.in_y[i] != null) in_y[i] = new String(wi.in_y[i]);
            else in_y[i] = null;
            if(wi.in_up_err[i] != null) in_up_err[i] = new String(wi.in_up_err[i]);
            else in_up_err[i] = null;
            if(wi.in_low_err[i] != null) in_low_err[i] = new String(wi.in_low_err[i]);
            else in_low_err[i] = null;
        }
        for(int i = 0; i < num_waves; i++){
            ...
            if(wi.shots != null) shots[i] = wi.shots[i];
            /*****/
            if(wi.evaluated != null) evaluated[i] = wi.evaluated[i];
            else evaluated[i] = false;
            if(wi.signals != null) signals[i] = wi.signals[i];
            if(wi.w_error != null) w_error[i] = wi.w_error[i];
            /*****/
        }
        in_upd_limits = wi.in_upd_limits;
        if(wi.in_xmin != null) in_xmin = new String(wi.in_xmin);
        else in_xmin = null;
        if(wi.in_ymin != null) in_ymin = new String(wi.in_ymin);
        else in_ymin = null;
        if(wi.in_xmax != null) in_xmax = new String(wi.in_xmax);
        else in_xmax = null;
        if(wi.in_ymax != null) in_ymax = new String(wi.in_ymax);
        else in_ymax = null;
        if(wi.in_timemax != null) in_timemax = new String(wi.in_timemax);
        else in_timemax = null;
        if(wi.in_timemin != null) in_timemin = new String(wi.in_timemin);
        else in_timemin = null;
        if(wi.in_shot != null) in_shot = new String(wi.in_shot);
        else in_shot = null;
        if(wi.experiment != null) experiment = new String(wi.experiment);
        else experiment = null;
        if(wi.in_title != null) in_title = new String(wi.in_title);
        else in_title = null;
        if(wi.in_xlabel != null) in_xlabel = new String(wi.in_xlabel);
        else in_xlabel = null;
        if(wi.in_ylabel != null) in_ylabel = new String(wi.in_ylabel);
        else in_ylabel = null;
        /*
         if(wi.in_upd_event != null)
             ...
             in_upd_event = null;
         */
        if(wi.in_def_node != null) in_def_node = new String(wi.in_def_node);
        else in_def_node = null;
        cin_upd_limits = wi.cin_upd_limits;
        if(wi.cin_xmin != null) cin_xmin = new String(wi.cin_xmin);
        else cin_xmin = null;
        if(wi.cin_ymin != null) cin_ymin = new String(wi.cin_ymin);
        else cin_ymin = null;
        if(wi.cin_xmax != null) cin_xmax = new String(wi.cin_xmax);
        else cin_xmax = null;
        if(wi.cin_ymax != null) cin_ymax = new String(wi.cin_ymax);
        else cin_ymax = null;
        if(wi.cin_timemax != null) cin_timemax = new String(wi.cin_timemax);
        else cin_timemax = null;
        if(wi.cin_timemin != null) cin_timemin = new String(wi.cin_timemin);
        else cin_timemin = null;
        if(wi.cin_shot != null) cin_shot = new String(wi.cin_shot);
        else cin_shot = null;
        if(wi.cexperiment != null) cexperiment = new String(wi.cexperiment);
        else cexperiment = null;
        if(wi.cin_title != null) cin_title = new String(wi.cin_title);
        else cin_title = null;
        if(wi.cin_xlabel != null) cin_xlabel = new String(wi.cin_xlabel);
        else cin_xlabel = null;
        if(wi.cin_ylabel != null) cin_ylabel = new String(wi.cin_ylabel);
        else cin_ylabel = null;
        if(wi.error != null) error = new String(wi.error);
        else error = null;
        if(wi.cin_upd_event != null) cin_upd_event = new String(wi.cin_upd_event);
        else cin_upd_event = null;
        if(wi.cin_def_node != null) cin_def_node = new String(wi.cin_def_node);
        else cin_def_node = null;
    ...
    public String GetDefaultValue(int i, boolean def_flag) {
        ...
        switch(i){
            ...
            case MdsWaveInterface.B_x_max:
                if(is_image) out = def_flag ? def_vals.xmax : cin_timemax;
                else out = def_flag ? def_vals.xmax : cin_xmax;
                break;
            case MdsWaveInterface.B_x_min:
                if(is_image) out = def_flag ? def_vals.xmin : cin_timemin;
                else out = def_flag ? def_vals.xmin : cin_xmin;
                break;
    ...
    public void UpdateDefault() {
        ...
        */
        ...
        if(is_image){
            ...
            in_xmax = cin_xmax;
        }else in_xmax = GetDefaultValue(bit, def_flag);
        bit = MdsWaveInterface.B_x_min;
        def_flag = ((defaults & (1 << bit)) == 1 << bit);
        if(is_image){
            in_timemin = GetDefaultValue(bit, def_flag);
            in_xmin = cin_xmin;
        }else in_xmin = GetDefaultValue(bit, def_flag);
    ...
    static public String containMainShot(String in_shot, String m_shot) {
        if(DEBUG.M){
            System.out.println("MdsWaveInterface.containMainShot(\"" + in_shot + "\", \"" + m_shot + "\")");
        }
        int idx;
        String out = in_shot;
        ...
        */
        if(UseDefaultShot()){
            if(main_shot_str != null && main_shot_str.length() != 0){
                ...
                if(curr_shots == null) error = "Main Shot evaluation error: " + ((jScopeWaveContainer)(wave.getParent())).getMainShotError(true);
            }else{
                if(def_vals.getIsEvaluated() && def_vals.shots != null) curr_shots = def_vals.shots;
                else{
        ...
        in_shot = c_shot_str;
        if(!UpdateShot(curr_shots)) previous_shot = "not defined";
    }
    ...
    public String getErrorString() // boolean brief_error)
    {
        ...
        if(!is_image){
            ...
            for(; i < w_error.length; i++){
                e = w_error[i];
                if(e != null){
                    if(!isAddSignal()) e = "<Wave " + (i + 1) + "> " + e;
                    if(brief_error){
                        idx = e.indexOf('\n');
                        if(idx < 0) idx = e.length();
                    }
                    if(full_error == null){
                        if(brief_error) full_error = e.substring(0, idx) + "\n";
                        else full_error = e + "\n";
                    }else{
                        if(brief_error) full_error = full_error + e.substring(0, idx) + "\n";
                        else full_error = full_error + e + "\n";
                    }
                }
            }
        }
        if(full_error == null && error != null) if(brief_error && error.indexOf("\n") != -1) full_error = error.substring(0, error.indexOf("\n"));
        else full_error = error;
        return full_error;
    }
    ...
    public synchronized void refresh() throws Exception {
        if(DEBUG.M){
            System.out.println("MdsWaveInterface.refresh()");
        }
        try{
            error = Update();
            if(error == null){
    ...
    public void ToFile(PrintWriter out, String prompt) throws IOException {
        ...
        */
        ...
        if(cnum_shot != 0){
            if(UseDefaultShot()) eval_shot = num_shot > 0 ? num_shot : 1;
    ...
    public static String mode1DCodeToString(int code) {
    ...
    public static int mode1DStringToCode(String mode) {
    ...
    public static String mode2DCodeToString(int code) {
    ...
    public static int mode2DStringToCode(String mode) {
    ...
    public void FromFile(Properties pr, String prompt, ColorMapDialog cmd) throws IOException {
        ...
        try{
            ...
            if(prop != null) height = new Integer(prop).intValue();
            prop = pr.getProperty(prompt + ".grid_mode");
            if(prop != null) in_grid_mode = new Integer(prop).intValue();
            ...
            if(continuousUpdateStr != null && continuousUpdateStr.trim().equals("1")) isContinuousUpdate = true;
            else isContinuousUpdate = false;
            prop = pr.getProperty(prompt + ".x_log");
            if(prop != null) x_log = new Boolean(prop).booleanValue();;
            prop = pr.getProperty(prompt + ".y_log");
            if(prop != null) y_log = new Boolean(prop).booleanValue();;
            prop = pr.getProperty(prompt + ".update_limits");
            if(prop != null) cin_upd_limits = new Boolean(prop).booleanValue();
            ...
            if(prop != null) is_image = new Boolean(prop).booleanValue();
            prop = pr.getProperty(prompt + ".keep_ratio");
            if(prop != null) keep_ratio = new Boolean(prop).booleanValue();
            prop = pr.getProperty(prompt + ".horizontal_flip");
            if(prop != null) horizontal_flip = new Boolean(prop).booleanValue();
            prop = pr.getProperty(prompt + ".vertical_flip");
            if(prop != null) vertical_flip = new Boolean(prop).booleanValue();
            prop = pr.getProperty(prompt + ".palette");
            if(prop != null){
                colorMap = cmd.getColorMap(prop);
                try{
                    prop = pr.getProperty(prompt + ".bitShift");
                    if(prop != null) colorMap.bitShift = Integer.parseInt(prop);
                    prop = pr.getProperty(prompt + ".bitClip");
                    if(prop != null) colorMap.bitClip = new Boolean(prop).booleanValue();
                }catch(Exception exc){
                    colorMap.bitShift = 0;
            ...
            if(prop != null){
                ...
                if(in_x == null || in_x.length == 0) in_x = new String[1];
                in_x[0] = x_str;
            }
            ...
            if(prop != null){
                ...
                if(in_x != null && in_x.length != 0) x_str = in_x[0];
                ...
                if(num_expr == 1){
                    ...
                    if(x_str != null) in_x[0] = new String(x_str);
                    else in_x[0] = null;
                }
            }
