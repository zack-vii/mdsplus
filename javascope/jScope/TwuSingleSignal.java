...
final public class TwuSingleSignal{
    ...
    private void DispatchConnectionEvent(ConnectionEvent e) {
        if(provider != null) provider.DispatchConnectionEvent(e);
    }
    ...
    private void setErrorString(String errmsg) {
        if(provider != null) provider.setErrorstring(errmsg);
    }
    ...
    private void fetchProperties() throws Exception {
        try{
            ...
            // error = false ;
            data = null;
            if(isAbscissa) fetch_X_Properties();
            else fetch_Y_Properties();
        }catch(Exception e){
            errorSource = e;
            // error = true ;
        }
    ...
    private void fetch_X_Properties() throws Exception {
        ...
        if(dim > 1 || dim < 0) throwError("Not a 1-dimensional signal !");
    ...
    private void fetch_Y_Properties() throws Exception {
        if(source == null) throwError("No input signal set !");
        String propsurl = TwuNameServices.GetSignalPath(source, shotOfTheProperties);
    ...
    // -----------------------------------------------------------------------------
    public float[] getData(TWUFetchOptions opt) throws IOException {
        setFetchOptions(opt);
        return getData();
    }
    ...
    public void setFetchOptions(TWUFetchOptions opt) throws IOException {
        doClip(opt);
        if(fetchOptions != null && fetchOptions.equalsForBulkData(opt)){ return; }
        fetchOptions = opt;
        data = null;
    }
    ...
    private void fetchBulkData() throws Exception {
        if(fetchOptions == null) throwError("unspecified fetch options (internal error)");
        if(fetchOptions.getTotal() == -1) doClip(fetchOptions); // just in case ...
    ...
    protected float[] doFetch(TWUFetchOptions opt) {
        ...
        TWUSignal bulk = new TWUSignal(properties, opt.getStart(), opt.getStep(), opt.getTotal());
    ...
    private float[] SimplifiedGetFloats(final TWUSignal bulk, final int n_point) {
        ...
        if(bulk.error()) setErrorString("Error reading Bulk Data");
        DispatchConnectionEvent(makeConnectionEvent(null, 0, 0));
    ...
    private void createScalarData() {
        ...
        if(properties != null && properties.LengthTotal() == 1){
            ...
            if(properties.getProperty("Signal.Minimum") != null) data = new float[]{(float)properties.Minimum()};
            else data = new float[]{0.0f};
