...
public class TWUSignal{
    ...
    // skipping algorithm.
    public TWUSignal(TWUProperties SigURL){
        this(SigURL, 0, 0, 0);
    }

    public TWUSignal(TWUProperties SigURL, int firstSample, int step, int maxSamples){
        boolean success = false;
        twup = SigURL;
        if(maxSamples <= 0) maxSamples = twup.LengthTotal();
        ...
        if(twup.Equidistant()) success = tryToConstruct(firstSample, step, maxSamples);
        if(!success) prepareToRead(firstSample, step, maxSamples);
    }
    ...
    private boolean tryToConstruct(int firstSample, int step, int maxSamples) {
        ...
        while(ix < toReturn) // or: (ix < maxSamples ) ???
        {
             ...
             */
            if(ydata[ix] > max) ydata[ix] = (float)max;
            else if(ydata[ix] < min) ydata[ix] = (float)min;
            ++ix;
        }
        sampleCount = ix;
        finished = true;
    ...
    private void prepareToRead(int firstSample, int step, int maxSamples) {
        try{
            error = false;
            StringBuffer bulk = new StringBuffer(twup.FQBulkName() + "?start=" + firstSample);
    ...
    public void tryToRead(final int samples2Try) {
        ...
        try{
            String s = null;
            while((samples2Try > thisAttempt++) && (samples2Read > sampleCount) && ((s = instream.readLine()) != null)){
            ...
            if(finished){
                ...
                if(sampleCount < samples2Read){
                    // Fill-up required
                    if(sampleCount == 0) ydata[sampleCount++] = 0.0F;
    ...
    public float[] getBulkData() {
        ...
        if(sampleCount < ydata.length){
            ...
            if(sampleCount > 0) System.arraycopy(ydata, 0, newydata, 0, sampleCount);
