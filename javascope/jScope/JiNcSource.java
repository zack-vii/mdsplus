...
public class JiNcSource implements JiDataSource{
    ...
    private static JiVar findVariable(String name, JiNcVar[] varray) {
        for(int i = 0; i < varray.length; ++i)
            if(name.equals(varray[i].getName())) return varray[i];
        return null;
    }
    ...
    private JiNcVar readAttribute(RandomAccessData in) throws IOException {
        ...
        JiNcVar nvar = new JiNcVar(this, name, dims, in.getFilePointer(), type, false);
    ...
    private JiNcVar[] readAttributes(RandomAccessData in) throws IOException {
        ...
        if(type != NcAttribute) throw new IOException("Bad file -- expected netCDF global atts");
    ...
    private void readDims(RandomAccessData in) throws IOException {
        ...
        if(type != NcDimension) throw new IOException("Bad file -- expected netCDF dimensions");
    ...
    private void readHeader(RandomAccessData in) throws IOException {
        ...
        readVars(in);
        if(DEBUG){
            System.out.println("readHeader: (recs, recsize): " + mRecs + " " + mRecordSize);
        }
    }

    private static String readString(RandomAccessData in) throws IOException {
        int nelems = in.readInt();
    ...
    private void readVars(RandomAccessData in) throws IOException {
        ...
        if(type == 0 && nelems == 0) return;
        if(type != NcVariable) throw new IOException("Bad file -- expected netCDF global atts");
    ...
    private static void toStringAtt(PrintStream out, JiVar parent, JiVar v) {
    ...
    private static void toStringData(PrintStream out, JiVar v) {
        try{
            byte bytes[] = null;
            char chars[] = null;
            short shorts[] = null;
    ...
    private static void toStringVar(PrintStream out, JiVar v) {
    ...
    private static void toStringVarData(PrintStream out, JiVar v) {
        out.print(v.getName() + " = ");
        toStringData(out, v);
    }
}
