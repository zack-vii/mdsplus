...
public class Waveform extends JComponent implements SignalListener{
    ...
    public Waveform(){
        ...
        play_timer = new javax.swing.Timer(100, new ActionListener(){
            @Override
            public void actionPerformed(ActionEvent evt) {
                frame = frames.getNextFrameIdx();
                if(frame == frames.getNumFrame() - 1) frame = 0;
                repaint();
                if(mode == MODE_POINT){
                    sendUpdateEvent();
                    if(send_profile) sendProfileEvent();
                }
            }
        });
    ...
    public static void SetDefaultColors() {
        ...
        if(colors != null && colors_name != null && (colors != COLOR_SET || colors_name != COLOR_NAME)) return;
        colors = COLOR_SET;
    ...
    static int ColorNameToIndex(String name) {
        ...
        if(name != null) for(int i = 0; i < COLOR_NAME.length; i++)
            if(name.toLowerCase().equals(COLOR_NAME[i].toLowerCase())) return i;
        return 0;
    }
    ...
    protected Dimension getWaveSize() {
        if(DEBUG.M) System.out.println("Waveform.getWaveSize()");
        Dimension dim = getSize();
        Insets i = getInsets();
        return new Dimension(dim.width - getRightSize() - i.top - i.bottom, dim.height - getBottomSize() - i.right - i.left);
    }
    ...
    public int GetColorIdx() {
        ...
        if(waveform_signal != null) return waveform_signal.getColorIdx();
        else if(frames != null) return frames.GetColorIdx();
        return 0;
    }
    ...
    public void SetColorIdx(int idx) {
        ...
        if(waveform_signal != null) waveform_signal.setColorIdx(idx);
        else{
            if(frames != null){
                idx = (idx == 0 ? 1 : idx);
    ...
    public String getSignalName() {
        ...
        if(is_image && frames != null) return frames.getName();
    ...
    public void SetMarkerStep(int step) {
        if(DEBUG.M){
            System.out.println("Waveform.SetMarkerStep(" + step + ")");
        }
    ...
    public int getSignalMode1D() {
        if(DEBUG.M){
            System.out.println("Waveform.getSignalMode1D()");
        }
        int mode = -1;
        if(waveform_signal != null) mode = waveform_signal.getMode1D();
        return mode;
    }

    public void setSignalMode1D(int mode) {
        if(DEBUG.M){
            System.out.println("Waveform.SetMarkerStep(" + mode + ")");
        }
        if(waveform_signal != null){
    ...
    public int getSignalMode2D() {
        ...
        int mode = -1;
        if(waveform_signal != null) mode = waveform_signal.getMode2D();
        return mode;
    }
    ...
    public int GetShowSignalCount() {
        return (waveform_signal != null) ? 1 : 0;
    }

    @Override
    public Dimension getMinimumSize() {
        if(DEBUG.M){
            System.out.println("Waveform.getMinimumSize()");
        }
        Insets i = getInsets();
    ...
    public void SetPointMeasure() {
        ...
        if(is_image){
            ...
            FindPoint(curr_x, curr_y, d, true);
            mark_point_x = curr_x = wave_point_x;
            mark_point_y = curr_y = wave_point_y;
        }
    }

    protected float convertX(int x) {
        if(DEBUG.M){
            System.out.println("Waveform.convertX(" + x + ")");
        }
        Dimension d = getWaveSize();
    ...
    protected float convertY(int y) {
        if(DEBUG.M){
            System.out.println("Waveform.convertY(" + y + ")");
        }
        Dimension d = getWaveSize();
    ...
    protected void setMouse() {
        ...
        addMouseListener(new MouseAdapter(){
            ...
            public void mousePressed(MouseEvent e) {
                if(getCursor().getType() == Cursor.WAIT_CURSOR) return;
                Insets i = getInsets();
                just_deselected = false;
                requestFocus();
                is_mb2 = is_mb3 = false;
                if((e.getModifiers() & Event.ALT_MASK) != 0) is_mb2 = true;
                else if((e.getModifiers() & Event.META_MASK) != 0) is_mb3 = true;
                if(mode == MODE_COPY && !is_mb3){
                    if(is_mb2){
                        if(!IsCopySelected()) sendPasteEvent();
                    }else sendCutEvent();
                    return;
                }
                ...
                int y = e.getY() - i.top;
                if(mode != MODE_POINT) update_timestamp++;
                start_x = end_x = prev_point_x = orig_x = x;
                start_y = end_y = prev_point_y = orig_y = y;
                dragging = true;
                first_set_point = true;
                setMousePoint();
                if(mode == MODE_PAN && waveform_signal != null) waveform_signal.StartTraslate();
                if(mode == MODE_POINT && waveform_signal != null) repaint();
                if((e.getModifiers() & Event.CTRL_MASK) != 0){
                    if(is_image){
                        if(frames != null && frames.GetFrameIdx() > 0) frame = frames.getLastFrameIdx();
                    }else{
                        Signal s = GetSignal();
                        if(s.getType() == Signal.TYPE_2D){
                            s.decShow();
                ...
                if((e.getModifiers() & Event.SHIFT_MASK) != 0){
                    if(is_image){
                        if(frames != null) frame = frames.getNextFrameIdx();
                    }else{
                        Signal s = GetSignal();
                        if(s.getType() == Signal.TYPE_2D){
                            s.incShow();
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                if((waveform_signal == null && !is_image) || (frames == null && is_image)) return;
                if(mode == MODE_ZOOM && x != orig_x && y != orig_y){
                    if(!is_image) performZoom();
                    not_drawn = true;
                }
                if(mode == MODE_ZOOM && zoom_on_mb1 && x == orig_x && y == orig_y && !is_image){
                    if((e.getModifiers() & Event.ALT_MASK) != 0) Resize(x, y, false);
                    else Resize(x, y, true);
                }
        ...
        addMouseMotionListener(new MouseMotionAdapter(){
            ...
            public void mouseDragged(MouseEvent e) {
                if(getCursor().getType() == Cursor.WAIT_CURSOR) return;
                if(waveform_signal == null && !is_image || is_mb2) return;
                ...
                if(is_image && frames != null){
                    ...
                    if(mode == MODE_ZOOM && !frames.contain(new Point(start_x, start_y), d)) return;
                    setMousePoint();
                }
                if(mode == MODE_ZOOM){
                    if(is_image && is_playing){
                    ...
                    if(curr_rect == null) curr_rect = new Rectangle(start_x, start_y, end_x - start_x, end_y - start_y);
                    else curr_rect.setBounds(start_x, start_y, end_x - start_x, end_y - start_y);
                    not_drawn = false;
                    repaint();
                }else{
                    curr_rect = null;
                ...
                if(mode == MODE_POINT){
                    ...
                    if(is_image && send_profile) sendProfileEvent();
                }
                if(mode == MODE_PAN && !is_image){
                    if(wm.x_log) pan_delta_x = wm.XValue(start_x, d) / wm.XValue(end_x, d);
                    else pan_delta_x = wm.XValue(start_x, d) - wm.XValue(end_x, d);
                    if(wm.y_log) pan_delta_y = wm.YValue(start_y, d) / wm.YValue(end_y, d);
                    else pan_delta_y = wm.YValue(start_y, d) - wm.YValue(end_y, d);
                    not_drawn = false;
                    repaint();
                }
            }
    ...
    public void SetMarker(int marker) {
        if(waveform_signal != null) waveform_signal.setMarker(marker);
    }
    ...
    public void SetInterpolate(boolean interpolate) {
        if(waveform_signal != null) waveform_signal.setInterpolate(interpolate);
    }
    ...
    public void SelectWave() {
        ...
        if(!is_select){
            ...
            if(!unselect_border.getBorderInsets(this).equals(select_border.getBorderInsets(this))) not_drawn = true;
    ...
    public void UpdateImage(Frames frames) {
        ...
        SetFrames(frames);
        // if (frames != null && frames.getNumFrame() > 0) frames.curr_frame_idx = 0;
        this.is_image = true;
        curr_rect = null;
        prev_point_x = prev_point_y = -1;
        not_drawn = true;
    ...
    protected void SetMode(int mode) {
        ...
        if(def_cursor == null) def_cursor = getCursor();
        switch(mode){
            case MODE_PAN:
                if(pan_enabled){
    ...
    void DrawWave(Dimension d) {
        ...
        if(curr_mode == Signal.MODE_STEP) num_points = waveform_signal.getNumPoints() * 2 - 1;
        else num_points = waveform_signal.getNumPoints();
    ...
    synchronized public void PaintImage(Graphics g, Dimension d, int print_mode) {
        ...
        grid = new Grid(xmax, ymax, xmin, ymin, x_log, y_log, grid_mode, x_label, y_label, title, wave_error, grid_step_x, grid_step_y, int_xlabel, int_ylabel, true);
        if(!is_min_size) grid.paint(g, d, this, null);
    }
    ...
    synchronized protected void PaintSignal(Graphics g, Dimension dim, int print_mode) {
        ...
        if(print_mode == NO_PRINT) d = getWaveSize();
        else d = getPrintWaveSize(dim);
        xmax = 1;
        ymax = 1;
        xmin = 0;
        ymin = 0;
        if(mode != MODE_PAN || dragging == false){
            if(waveform_signal != null){// TACON MOSTRUOSO per gestire il fatto che jScope vede solo gli offsets nei times!!!!
                xmax = MaxXSignal();
                xmin = MinXSignal();
                ymax = MaxYSignal();
                ymin = MinYSignal();
        ...
        if(resizing || grid == null || wm == null || change_limits){
            change_limits = false;
            grid = new Grid(xmax, ymax, xmin, ymin, x_log, y_log, grid_mode, orizLabel, vertLabel, sigTitle, wave_error, grid_step_x, grid_step_y, int_xlabel, int_ylabel, reversed);
            ...
            grid.updateValues(orizLabel, vertLabel, sigTitle, wave_error, grid_step_x, grid_step_y, int_xlabel, int_ylabel, reversed);
            grid.setLabels(sigTitle, orizLabel, vertLabel);
        }
        if(waveform_signal != null) grid.setXaxisHMS(waveform_signal.isLongX());
        if(!copy_selected || print_mode != NO_PRINT){
            if(reversed && print_mode == NO_PRINT) g.setColor(Color.black);
            else g.setColor(Color.white);
        }else g.setColor(Color.lightGray);
        g.fillRect(1, 1, d.width - 2, d.height - 2);
        if(waveform_signal != null){
            wave_b_box = new Rectangle(wm.XPixel(MinXSignal(), d), wm.YPixel(MaxYSignal(), d), wm.XPixel(MaxXSignal(), d) - wm.XPixel(MinXSignal(), d) + 1, wm.YPixel(MinYSignal(), d) - wm.YPixel(MaxYSignal(), d) + 1);
            if(print_mode == NO_PRINT){
                g.clipRect(curr_display_limits.width, 0, d.width - curr_display_limits.width, d.height - curr_display_limits.height);
            }
        ...
        if(print_mode == PRINT && mode == MODE_POINT){
            ...
            if(p != null){
                ...
                if(crosshair_color != null) g.setColor(crosshair_color);
                g.drawLine(0, p.y, d.width, p.y);
                g.drawLine(p.x, 0, p.x, d.height);
                g.setColor(prev_color);
            }
        }
        if(!is_min_size && grid != null) grid.paint(g, d, this, wm);
    }
    ...
    public void sendUpdateEvent() {
        if(DEBUG.M){
            System.out.println("Waveform.sendUpdateEvent()");
        }
        ...
        if(is_image && frames != null && frames.getNumFrame() != 0){
            ...
            we = new WaveformEvent(this, WaveformEvent.POINT_UPDATE, p.x, p.y, frames.GetFrameTime(), 0, frames.getPixel(frames.GetFrameIdx(), p.x, p.y), 0);
            if(frame_type == FrameData.BITMAP_IMAGE_32 || frame_type == FrameData.BITMAP_IMAGE_16 || frame_type == FrameData.BITMAP_IMAGE_8) we.setPointValue(frames.getPointValue(frames.GetFrameIdx(), p.x, p.y));
            we.setFrameType(frame_type);
            dispatchWaveformEvent(we);
        }else if(waveform_signal != null && wm != null){
            ...
            event_id = show_measure ? WaveformEvent.MEASURE_UPDATE : WaveformEvent.POINT_UPDATE;
    ...
    public void sendProfileEvent() {
        ...
        if(frames != null && frames.getNumFrame() != 0){
            ...
            if(frame_type == FrameData.BITMAP_IMAGE_32 || frame_type == FrameData.BITMAP_IMAGE_16){
                we = new WaveformEvent(this, p.x, p.y, (float)(Math.round(frames.GetFrameTime() * 10000) / 10000.), frames.getName(), frames.getValuesX(p.y), frames.getStartPixelX(), frames.getValuesY(p.x), frames.getStartPixelY());
                if(show_measure){
                ...
                we = new WaveformEvent(this, p.x, p.y, (float)(Math.round(frames.GetFrameTime() * 10000) / 10000.), frames.getName(), frames.getPixelsX(p.y), frames.getStartPixelX(), frames.getPixelsY(p.x), frames.getStartPixelY());
                if(show_measure){
                    Point p_pos = new Point((int)mark_point_x, (int)mark_point_y);
                    Point mp = frames.getFramePoint(p_pos, d);
    ...
    private void ImageActions(Graphics g, Dimension d) {
        ...
        if(frames != null && frames.getNumFrame() != 0 && (mode == MODE_POINT || mode == MODE_ZOOM) && !not_drawn && !is_min_size || (frames != null && send_profile && prev_frame != frame)){
            // if(!is_playing )
            {
                if(mode == MODE_POINT){
                    Color prev_color = g.getColor();
    ...
    private void SignalActions(Graphics g, Dimension d) {
        if(DEBUG.M){
            System.out.println("Waveform.SignalActions(" + g + ", " + d + ")");
        }
        double curr_x, curr_y;
        if(waveform_signal != null && mode == MODE_POINT && !not_drawn && !is_min_size && wm != null){
            ...
            first_set_point = false;
            if(p != null){
                if(curr_point_y != Double.NaN) p.y = wm.YPixel(curr_point_y, d);
                curr_x = wave_point_x;
                curr_y = wave_point_y;
                Color prev_color = g.getColor();
                if(crosshair_color != null) g.setColor(crosshair_color);
        ...
        if(mode == MODE_PAN && dragging && waveform_signal != null){
            ...
            wm = new WaveformMetrics(MaxXSignal(), MinXSignal(), MaxYSignal(), MinYSignal(), curr_display_limits, d, wm.x_log, wm.y_log, 0, 0);
            g.setColor(reversed ? Color.black : Color.white);
            g.fillRect(1, 1, d.width - 2, d.height - 2);
    ...
    public void paintComponent(Graphics g) {
        ...
        Dimension d = getSize();
        paint(g, d, NO_PRINT);
        if(mode == MODE_POINT && send_profile) sendProfileEvent();
    }
    boolean appendPaintFlag = false;

    synchronized public void appendPaint(Graphics g, Dimension d) {
        if(DEBUG.M){
            System.out.println("Waveform.appendPaint(" + g + ", " + d + ")");
        }
        setFont(g);
        g.setColor(Color.black);
        if(!is_image){
            ...
            }else PaintSignal(g, d, NO_PRINT);
        }else PaintImage(g, d, NO_PRINT);
    }
    ...
    synchronized public void paint(Graphics g, Dimension d, int print_mode) {
        ...
        setFont(g);
        Dimension dim;
        if(not_drawn || prev_width != d.width || prev_height != d.height || execute_print || (is_image && prev_frame != frame) || appendPaintFlag){
            ...
            if(print_mode == NO_PRINT){
                ...
                dim = getWaveSize();
                is_min_size = (dim.width < MIN_W || dim.height < MIN_H);
                if(bug_image){
                    bug_image = false;
            ...
            if(is_image) PaintImage(off_graphics, d, print_mode);
            else PaintSignal(off_graphics, d, print_mode);
            if(print_mode == NO_PRINT){
                off_graphics.translate(-i.right, -i.top);
        ...
        if(!(mode == MODE_PAN && dragging && waveform_signal != null) || is_image) if(off_image != null) g.drawImage(off_image, 0, 0, this);
        g.translate(i.right, i.top);
        if(mode == MODE_ZOOM) if(curr_rect != null){
            if(is_image && crosshair_color != null) g.setColor(crosshair_color);
            else g.setColor(reversed ? Color.white : Color.black);
            g.drawRect(curr_rect.x, curr_rect.y, curr_rect.width, curr_rect.height);
        }
        if(is_image) ImageActions(g, d = getWaveSize());
        else SignalActions(g, d = getWaveSize());
    ...
    protected boolean FindPoint(Signal s, double curr_x, double curr_y) {
        if(DEBUG.M){
            System.out.println("Waveform.FindPoint(" + s + ", " + curr_x + ", " + curr_y + ")");
        }
        wave_point_x = wave_point_y = Double.NaN;
        ...
        if(curr_x > s.getCurrentXmax() || curr_x < s.getCurrentXmin() || idx == s.getNumPoints() - 1){
            wave_point_y = s.getY(idx);
            wave_point_x = s.getX(idx);
        }else{
            if(s.getMarker() != Signal.NONE && !s.getInterpolate() && s.getMode1D() != Signal.MODE_STEP || s.findNaN()){
                ...
                if(curr_x < val){
                    wave_point_y = s.getY(idx + (increase ? 0 : 1));
                    wave_point_x = s.getX(idx + (increase ? 0 : 1));
                }else{
                    wave_point_y = s.getY(idx + (increase ? 1 : 0));
                    wave_point_x = s.getX(idx + (increase ? 1 : 0));
                }
            }else{
                wave_point_x = curr_x;
                try{
                    if(s.getMode1D() == Signal.MODE_STEP){
                        ...
                        wave_point_y = s.getY(idx) + (s.getY(idx + 1) - s.getY(idx)) * (wave_point_x - s.getX(idx)) / (s.getX(idx + 1) - s.getX(idx));
                    }
    ...
    protected Point FindPoint(double curr_x, double curr_y, boolean is_first) {
        if(!FindPoint(waveform_signal, curr_x, curr_y)) return null;
        Dimension d = getWaveSize();
        return new Point(wm.XPixel(wave_point_x, d), wm.YPixel(wave_point_y, d));
    }

    protected Point FindPoint(Signal signal, double curr_x, double curr_y, Dimension d) {
        if(!FindPoint(signal, curr_x, curr_y)) return null;
        if(signal.getType() == Signal.TYPE_2D && !(signal.getMode2D() == Signal.MODE_PROFILE)) d = getWaveSize();
        return new Point(wm.XPixel(wave_point_x, d), wm.YPixel(wave_point_y, d));
    }

    protected Point FindPoint(double curr_x, double curr_y, Dimension d, boolean is_first) {
        if(!FindPoint(waveform_signal, curr_x, curr_y)) return null;
        if(waveform_signal.getType() == Signal.TYPE_2D && !(waveform_signal.getMode2D() == Signal.MODE_PROFILE)) d = getWaveSize();
        return new Point(wm.XPixel(wave_point_x, d), wm.YPixel(wave_point_y, d));
    }
    ...
    protected boolean DrawFrame(Graphics g, Dimension d, int frame_idx) {
        if(DEBUG.M){
            System.out.println("Waveform.DrawFrame(" + g + ", " + d + ", " + frame_idx + ")");
        }
        wave_error = null;
        Object img;
    ...
    protected void DrawImage(Graphics g, Object img, Dimension dim, int type) {
        if(DEBUG.M){
            System.out.println("Waveform.DrawImage(" + g + ", " + img + ", " + dim + ", " + type + ")");
        }
        ...
        if(type != FrameData.JAI_IMAGE){
            ...
            /*
             * if (r == null) { g2.drawImage( (Image) img, 1, 1, dim.width, dim.height, this); } else { g2.drawImage( (Image) img, 1, 1, dim.width, dim.height, r.x, r.y, r.x + r.width, r.y + r.height, this); }
             */
        }else{
    ...
    protected void drawSignalContour(Signal s, Graphics g, Dimension d) {
        if(DEBUG.M){
            System.out.println("Waveform.drawSignalContour(" + s + ", " + g + ", " + d + ")");
        }
    ...
    public void drawContourLevel(Vector<Vector<Point2D.Double>> cOnLevel, Graphics g, Dimension d) {
        if(DEBUG.M){
            System.out.println("Waveform.drawContourLevel(" + cOnLevel + ", " + g + ", " + d + ")");
        }
    ...
    protected void drawSignal(Signal s, Graphics g, Dimension d) {
        if(DEBUG.M){
            System.out.println("Waveform.drawSignal(" + s + ", " + g + ", " + d + ")");
        }
        if(s.getType() == Signal.TYPE_2D){
            switch(s.getMode2D()){
                case Signal.MODE_IMAGE:
    ...
    protected void drawMarkers(Signal s, Graphics g, Dimension d) {
        if(DEBUG.M){
            System.out.println("Waveform.drawMarkers(" + s + ", " + g + ", " + d + ")");
        }
    ...
    public void setXlimits(float xmin, float xmax) {
        if(DEBUG.M){
            System.out.println("Waveform.setXlimits(" + xmin + ", " + xmax + ")");
        }
        if(waveform_signal != null) try{
    ...
    public void setYlimits(float ymin, float ymax) {
        if(DEBUG.M){
            System.out.println("Waveform.setYlimits(" + ymin + ", " + ymax + ")");
        }
        if(waveform_signal != null) waveform_signal.setYlimits(ymin, ymax);
    }
    ...
    protected double MinXSignal() {
        ...
        if(waveform_signal == null) return 0.;
    ...
    protected double MaxXSignal() {
        ...
        if(waveform_signal == null) return 1.;
    ...
    protected double MinYSignal() {
        ...
        if(waveform_signal == null) return 0.;
        if(waveform_signal.getYmax() <= waveform_signal.getYmin()) return waveform_signal.getYmin() - 1E-3 - Math.abs(waveform_signal.getYmax());
    ...
    protected double MaxYSignal() {
        ...
        if(waveform_signal == null) return 1.;
        if(waveform_signal.getYmax() <= waveform_signal.getYmin()) return waveform_signal.getYmax() + 1E-3 + Math.abs(waveform_signal.getYmax());
    ...
    protected void drawMarkers(Graphics g, Point pnt[], int n_pnt, int marker, int step, int mode) {
        if(DEBUG.M){
            System.out.println("Waveform.drawMarkers(" + g + ", " + pnt + ", " + n_pnt + ", " + marker + ", " + step + ", " + mode + ")");
        }
        for(int i = 0; i < n_pnt; i += step){
            ...
            switch(marker){
                ...
                case Signal.TRIANGLE:
                    g.drawLine(pnt[i].x - marker_width / 2, pnt[i].y + marker_width / 2, pnt[i].x, pnt[i].y - marker_width / 2);
                    g.drawLine(pnt[i].x + marker_width / 2, pnt[i].y + marker_width / 2, pnt[i].x, pnt[i].y - marker_width / 2);
                    g.drawLine(pnt[i].x - marker_width / 2, pnt[i].y + marker_width / 2, pnt[i].x + marker_width / 2, pnt[i].y + marker_width / 2);
                    break;
                case Signal.CROSS:
    ...
    protected void drawMarkers(Graphics g, Vector<Polygon> segments, int marker, int step, int mode) {
        if(DEBUG.M){
            System.out.println("Waveform.drawMarkers(" + g + ", " + segments + ", " + marker + ", " + step + ", " + mode + ")");
        }
        Polygon currPolygon;
        int pntX[];
        int pntY[];
            ...
            for(int i = 0; i < currPolygon.npoints; i += step){
                ...
                switch(marker){
                    ...
                    case Signal.SQUARE:
                        ...
                        break;
                    case Signal.TRIANGLE:
                        g.drawLine(pntX[i] - marker_width / 2, pntY[i] + marker_width / 2, pntX[i], pntY[i] - marker_width / 2);
                        g.drawLine(pntX[i] + marker_width / 2, pntY[i] + marker_width / 2, pntX[i], pntY[i] - marker_width / 2);
                        g.drawLine(pntX[i] - marker_width / 2, pntY[i] + marker_width / 2, pntX[i] + marker_width / 2, pntY[i] + marker_width / 2);
                        break;
                    case Signal.CROSS:
    ...
    void drawError(Signal sig, Graphics g, Dimension d) {
        if(DEBUG.M){
            System.out.println("Waveform.drawError(" + sig + ", " + g + ", " + d + ")");
        }
        ...
        for(int i = 0; i < sig.getNumPoints(); i++){
            try{
                ...
                if(!sig.hasAsymError()) low = wm.YPixel(sig.getY(i) - up_error[i], d);
                else low = wm.YPixel(sig.getY(i) - low_error[i], d);
    ...
    public synchronized void UpdatePoint(double curr_x, double curr_y) {
        if(DEBUG.M){
            System.out.println("Waveform.UpdatePoint(" + curr_x + ", " + curr_y + ")");
        }
        Dimension d = getWaveSize();
    ...
    protected void ReportLimits(ZoomRegion r, boolean add_undo) {
        ...
        if(add_undo){
            ZoomRegion r_prev = new ZoomRegion(waveform_signal.getXmin(), waveform_signal.getXmax(), waveform_signal.getYmax(), waveform_signal.getYmin());
            undo_zoom.addElement(r_prev);
        }else{
            undo_zoom.removeElement(r);
            if(undo_zoom.size() == 0) waveform_signal.unfreeze();
        }
        ...
        change_limits = true;
        if(add_undo) waveform_signal.freeze();
    }

    public void Autoscale() {
        if(DEBUG.M){
            System.out.println("Waveform.Autoscale()");
        }
        if(is_image && frames != null && frames.getNumFrame() != 0) frames.Resize();
        else{
    ...
    public void SetScale(Waveform w) {
        if(DEBUG.M){
            System.out.println("Waveform.SetScale(" + w + ")");
        }
        if(waveform_signal == null) return;
    ...
    protected void Resize(int x, int y, boolean enlarge) {
        if(DEBUG.M){
            System.out.println("Waveform.Resize(" + x + ", " + y + ", " + enlarge + ")");
        }
        Dimension d = getWaveSize();
        double curr_x = wm.XValue(x, d), curr_y = wm.YValue(y, d);
        double new_xmax, new_xmin, new_ymax, new_ymin;
        double factor = enlarge ? .5 : 2.;
        new_xmin = curr_x - (curr_x - wm.XMin()) * factor;
        new_xmax = curr_x + (wm.XMax() - curr_x) * factor;
        new_ymin = curr_y - (curr_y - wm.YMin()) * factor;
        new_ymax = curr_y + (wm.YMax() - curr_y) * factor;
        ReportLimits(new ZoomRegion(new_xmin, new_xmax, new_ymax, new_ymin), true);
        not_drawn = true;
    }

    public void undoZoom() {
        if(undo_zoom.size() == 0) return;
        ReportLimits(undo_zoom.lastElement(), false);
        not_drawn = true;
        repaint();
    }
    ...
    public void performZoom() {
        if(wm == null) return;
        Dimension d = getWaveSize();
        double start_xs = wm.XValue(start_x, d), end_xs = wm.XValue(end_x, d);
        double start_ys = wm.YValue(start_y, d), end_ys = wm.YValue(end_y, d);
        ReportLimits(new ZoomRegion(start_xs, end_xs, start_ys, end_ys), true);
    }
    ...
    void ReportChanges() {
        ...
        if(waveform_signal != null) NotifyZoom(waveform_signal.getXmin(), waveform_signal.getXmax(), waveform_signal.getYmin(), waveform_signal.getYmax(), update_timestamp);
    }
    ...
    public void SetReversed(boolean reversed) {
        ...
        if(grid != null) grid.SetReversed(reversed);
    }
    ...
    public void setColorMap(ColorMap cm) {
        this.colorMap = cm;
        if(frames != null) frames.setColorMap(cm);
        not_drawn = true;
        repaint();
    }
    ...
    public void applyColorModel(ColorMap cm) {
        if(cm == null) return;
        if(frames != null) frames.applyColorModel(cm);
        else this.colorMap = cm;
        not_drawn = true;
        repaint();
    }
    ...
    public synchronized void addWaveformListener(WaveformListener l) {
        if(l != null) waveform_listener.addElement(l);
    }

    public synchronized void removeWaveformListener(WaveformListener l) {
        if(l != null) waveform_listener.removeElement(l);
    }

    protected synchronized void dispatchWaveformEvent(WaveformEvent e) {
        if(e == null || !event_enabled || waveform_listener == null) return;
