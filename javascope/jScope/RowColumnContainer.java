...
public class RowColumnContainer extends JComponent{
    ...
    class Btm extends Component{
        ...
        public void paint(Graphics g) {
            ...
            if(d.width > d.height) setCursor(new Cursor(Cursor.N_RESIZE_CURSOR));
    ...
    private void setListener(Component b) {
        b.addMouseListener(new MouseAdapter(){
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                if(ob instanceof Btm) if(!((m_button & MouseEvent.BUTTON2_MASK) == MouseEvent.BUTTON2_MASK)) row_col_layout.ResizeRowColumn(ob, e.getPoint().x, e.getPoint().y);
            }
        ...
        b.addMouseMotionListener(new MouseMotionAdapter(){
            ...
            public void mouseDragged(MouseEvent e) {
                Component ob = e.getComponent();
                int m_button = e.getModifiers();
                if(!((m_button & MouseEvent.BUTTON2_MASK) == MouseEvent.BUTTON2_MASK)) row_col_layout.DrawResize(ob, e.getPoint().x, e.getPoint().y);
            }
        });
    }
    ...
    public void add(Component c[]) {
        int i, j, k;
        if(c.length != getComponentNumber()) throw new IllegalArgumentException("Invalid component number");
    ...
    */
    ...
    private Point getRealPosition(Point in_pos) {
        for(int i = 0; i < real_position.size(); i += 2){
            ...
            if(in_pos.x == p.x && in_pos.y == p.y) return real_position.elementAt(i + 1);
        }
        return null;
    }
    ...
    public void add(Component c, int row, int col) {
        ...
        if(getGridComponent(row, col) != null) throw new IllegalArgumentException("Component already added in this position");
        if(col > rows.length){
            if(row != 1) rrow = 1;
        ...
        setListener(b);
        if(rrow != row || rcol != col) setRealPosition(new Point(col, row), new Point(rcol, rrow));
    }
    ...
    */
    ...
    public Component getGridComponent(int row, int col) {
        ...
        if(p != null){
            col = p.x;
            row = p.y;
        }else if(positionOverwrite(curr_pos)) return null;
    ...
    public int getComponentIndex(Component c) {
        ...
        if(idx < getGridComponentCount()) return idx + 1;
        return -1;
    }
    ...
    public void removeComponent(int row, int col) {
        ...
        for(int i = row + 1; i <= rows[col - 1]; i++){
            ...
            if(!found) setRealPosition(new Point(col, i + 1), new Point(col, i));
        }
        rows[col - 1]--;
