...
class TwuNameServices{
    ...
    public static String legendString(WaveInterface wi, String signalURL, long shot) {
        ...
        if(ix > 0) return hashed2shot(signalURL.substring(ix), shot);
    ...
    static protected String GetSignalPath(String internalSignalURL, long shot) {
        if(IsFullURL(internalSignalURL)) return internalSignalURL;
        // Hashed_URLs
        // Check if signal path is in the format
        // //url_server_address/experiment/shotGroup/#####/signal_path
        if(isHashedURL(internalSignalURL)) return hashed2shot(internalSignalURL, shot);
        ...
        if(p_url == null) p_url = default_provider_url;
