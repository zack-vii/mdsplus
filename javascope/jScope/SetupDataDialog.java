...
final class SetupDataDialog extends JDialog implements ActionListener, ItemListener, KeyListener, WaveformListener{
    ...
    class Data{
        ...
        public boolean equals(Data ws) {
            if(x_expr != null){
                if(!x_expr.equals(ws.x_expr)) return false;
            }else if(ws.x_expr != null && ws.x_expr.length() != 0) return false;
            if(y_expr != null){
                if(!y_expr.equals(ws.y_expr)) return false;
            }else if(ws.y_expr != null && ws.y_expr.length() != 0) return false;
            if(label != null){
                if(!label.equals(ws.label)) return false;
            }else if(ws.label != null && ws.label.length() != 0) return false;
            return true;
        }

        public void copy(Data ws) {
            if(ws.label != null) label = new String(ws.label);
            if(ws.x_expr != null) x_expr = new String(ws.x_expr);
            if(ws.y_expr != null) y_expr = new String(ws.y_expr);
            if(ws.up_err != null) up_err = new String(ws.up_err);
            if(ws.low_err != null) low_err = new String(ws.low_err);
            shot = ws.shot;
    ...
    class SError extends JDialog implements ActionListener{
        ...
        public void setError(Data ws_in) {
            ws = ws_in;
            resetError();
            if(ws.up_err != null) e_up.setText(ws.up_err);
            if(ws.up_err != null) e_low.setText(ws.low_err);
        }
    ...
    class SList extends JPanel implements ItemListener{
        ...
        public SList(){
            ...
            sig_list.addKeyListener(new KeyAdapter(){
                ...
                public void keyPressed(KeyEvent e) {
                    ...
                    if(key == KeyEvent.VK_DELETE) removeSignalSetup();
                }
            });
        ...
        public int getSignalSelect() {
            if(sig_list.getModel().getSize() == 2 && image_b.isSelected()) return 0;
            return sel_signal;
        }

        private void signalSelect(int sig) {
            if(sig + 1 >= sig_list.getModel().getSize()) return;
            setSignalSelect(sig);
            int id = getSignalSelect() + 1;
            sig_list.setSelectedIndex(id);
            sig_list.ensureIndexIsVisible(id);
            if(sig >= 0) if(getSignalSelect() < signals.size()) putSignalSetup(signals.elementAt(getSignalSelect()));
            else resetSignalSetup();
        ...
        private void setOptionState(boolean state) {
            ...
            if(getSignalSelect() == -1) marker_step_t.setEditable(false);
        }

        public int getNumShot() {
            if(shots != null) return shots.length;
            return 0;
        }
        ...
        public int findSignalSetup(Data ws) {
            for(int i = 0; i < signals.size(); i++)
                if((signals.elementAt(i)).equals(ws)) return i;
            return -1;
        }
        ...
        public void removeSignalSetup() {
            ...
            if(shots != null && shots.length > 0) num_shot = shots.length;
            if(getSignalSelect() != -1){
                start_idx = (getSignalSelect() / num_shot) * num_shot; // intern division
        ...
        private void setMarkerTextState(int marker_idx) {
            if(marker_idx > 0 && marker_idx < 5) marker_step_t.setEditable(true);
            else{
                marker_step_t.setText("1");
        ...
        public void putSignalSetup(Data ws) {
            if(ws.label != null) signal_label.setText(ws.label);
            else signal_label.setText("");
            if(ws.x_expr != null) x_expr.setText(ws.x_expr);
            else x_expr.setText("");
            if(ws.y_expr != null) y_expr.setText(ws.y_expr);
            else y_expr.setText("");
            ...
            if(error_w.isVisible()) error_w.setError(ws);
        }
        ...
        public void reset() {
            signalSelect(-1);
            if(signals.size() != 0) signals.removeAllElements();
        }
        ...
        public void init(WaveInterface wi) {
            ...
            if(wi != null){
                ...
                for(int i = 0; i < wi.num_waves; i++){
                    ...
                    else ws.shot = UNDEF_SHOT;
                    addSignalSetup(ws);
                }
                signalListRefresh();
            }
            if(getSignalSelect() == -1 && wi.num_waves > 0) setSignalSelect(0);
        ...
        public void addSignals() {
            int idx, color_idx = 0;
            Data ws;
            if(y_expr.getText().length() == 0) return;
            ws = getSignalSetup();
        ...
        public boolean evaluateShotList(String in_shot) throws IOException {
            if(shots != null && shots.length != 0) list_num_shot = shots.length;
            else list_num_shot = 1;
            ...
            if(shots.equals(new_shots)){ return false; }
            shots = new_shots;
        ...
        public void signalsRefresh() {
            ...
            for(int j = 0; j < num_sig; j += n_shot){
                for(int i = 0; i < n_shot; i++){
                    ...
                    if(shots != null) signals.elementAt(k).shot = shots[i];
                    else signals.elementAt(k).shot = UNDEF_SHOT;
                    k++;
                }
        ...
        public void updateSignals() {
            ...
            int num_shot = 1;
            int num_signal = signals.size();
            if(findSignalSetup(getSignalSetup()) != -1) return;
            if(getSignalSelect() != -1){
                if(shots != null && shots.length > 0) num_shot = shots.length;
        ...
        private void signalListAdd(Data ws) {
            if(ws.shot != UNDEF_SHOT){
                if(ws.x_expr == null || ws.x_expr.length() == 0) list_model.addElement("Y : " + getExpressionList(ws.y_expr) + " Shot : " + ws.shot);
                else list_model.addElement("Y : " + getExpressionList(ws.y_expr) + " X : " + getExpressionList(ws.x_expr) + " Shot : " + ws.shot);
            }else{
                if(ws.x_expr == null || ws.x_expr.length() == 0) list_model.addElement("Y : " + getExpressionList(ws.y_expr) + " Shot : Undef");
                else list_model.addElement("Y : " + getExpressionList(ws.y_expr) + " X : " + getExpressionList(ws.x_expr) + " Shot : Undef");
            }
        }
        ...
        private void signalListReplace(int idx, Data ws) {
            if(ws.shot != UNDEF_SHOT){
                if(ws.x_expr == null || ws.x_expr.length() == 0) list_model.setElementAt("Y : " + getExpressionList(ws.y_expr) + " Shot : " + ws.shot, idx);
                else list_model.setElementAt("Y : " + getExpressionList(ws.y_expr) + " X : " + getExpressionList(ws.x_expr) + " Shot : " + ws.shot, idx);
            }else{
                if(ws.x_expr == null || ws.x_expr.length() == 0) list_model.setElementAt("Y : " + getExpressionList(ws.y_expr) + " Shot : Undef", idx);
                else list_model.setElementAt("Y : " + getExpressionList(ws.y_expr) + " X : " + getExpressionList(ws.x_expr) + " Shot : Undef", idx);
            }
        }
        ...
        public void updateList() throws IOException {
            if(getSignalSelect() == -1){
                ...
                if(y_expr.getText().trim().length() != 0) signalList.updateSignals();
                else removeSignalSetup();
            }
        ...
        */
        ...
        public void itemStateChanged(ItemEvent e) {
            Object ob = e.getSource();
            if(ob instanceof JCheckBox) DefaultButtonChange(ob);
            if(getSignalSelect() == -1 || image_b.isSelected()) return;
            if(ob == marker){
    ...
    class ExpandExp extends JDialog implements ActionListener{
        ...
        public void setExpressionString(String x, String y) {
            ...
            if(x != null) x_expr.setText(x);
            if(y != null) y_expr.setText(y);
        }
    ...
    public void Show(Waveform w, int col, int row) {
        ...
        wi = new MdsWaveInterface(wave, ((MdsWaveInterface)wave.wi).dp, ((MdsWaveInterface)wave.wi).def_vals, wave.wi.cache_enabled);
    ...
    private void resetDefaultFlags() {
        ...
        if(wi.is_image) wi.defaults = wi.defaults & ~((1 << MdsWaveInterface.B_y_max) + (1 << MdsWaveInterface.B_y_min));
        ...
        for(int i = 0; i < 32; i++){
            switch(i){
                ...
                case MdsWaveInterface.B_x_max:
                    if(image_b.isSelected()) time_max_b.setSelected(((flags & (1 << i)) == 1 << i) ? true : false);
                    else x_max_b.setSelected(((flags & (1 << i)) == 1 << i) ? true : false);
                    break;
                case MdsWaveInterface.B_x_min:
                    if(image_b.isSelected()) time_min_b.setSelected(((flags & (1 << i)) == 1 << i) ? true : false);
                    else x_min_b.setSelected(((flags & (1 << i)) == 1 << i) ? true : false);
                    break;
    ...
    public boolean isChanged(Data s[]) {
        ...
        if(!jScopeFacade.equalsString(title.getText(), wave_wi.cin_title)) return true;
        if(!jScopeFacade.equalsString(x_max.getText(), wave_wi.cin_xmax)) return true;
        if(!jScopeFacade.equalsString(x_min.getText(), wave_wi.cin_xmin)) return true;
        if(image_b.isSelected()){
            if(!jScopeFacade.equalsString(time_max.getText(), wave_wi.cin_timemax)) return true;
            if(!jScopeFacade.equalsString(time_min.getText(), wave_wi.cin_timemin)) return true;
        }
        if(!jScopeFacade.equalsString(x_label.getText(), wave_wi.cin_xlabel)) return true;
        if(x_log.isSelected() != wave_wi.x_log) return true;
        if(upd_limits.isSelected() != wave_wi.cin_upd_limits) return true;
        if(!jScopeFacade.equalsString(y_max.getText(), wave_wi.cin_ymax)) return true;
        if(!jScopeFacade.equalsString(y_min.getText(), wave_wi.cin_ymin)) return true;
        if(!jScopeFacade.equalsString(y_label.getText(), wave_wi.cin_ylabel)) return true;
        if(y_log.isSelected() != wave_wi.y_log) return true;
        // if(!main_scope.equalsString(shot.getText(), wave_wi.cin_shot)) return true;
        if(!jScopeFacade.equalsString(shot.getText(), wave_wi.in_shot)) return true;
        if(!jScopeFacade.equalsString(upd_event.getText(), wave_wi.cin_upd_event)) return true;
        if(!jScopeFacade.equalsString(def_node.getText(), wave_wi.cin_def_node)) return true;
        if(!jScopeFacade.equalsString(experiment.getText(), wave_wi.cexperiment)) return true;
    ...
    private void updateGlobalWI() {
        if(!experiment_b.isSelected() && !jScopeFacade.equalsString(experiment.getText(), wi.cexperiment)) wi.cexperiment = experiment.getText();
        if(!shot_b.isSelected() && !jScopeFacade.equalsString(shot.getText(), wi.cin_shot)) wi.cin_shot = shot.getText();
        if(!upd_event_b.isSelected() && !jScopeFacade.equalsString(upd_event.getText(), wi.cin_upd_event)) wi.cin_upd_event = upd_event.getText();
        if(!def_node_b.isSelected() && !jScopeFacade.equalsString(def_node.getText(), wi.cin_def_node)) wi.cin_def_node = def_node.getText();
        if(!time_max_b.isSelected() && !jScopeFacade.equalsString(time_max.getText(), wi.cin_timemax)) wi.cin_timemax = time_max.getText();
        if(!time_min_b.isSelected() && !jScopeFacade.equalsString(time_min.getText(), wi.cin_timemin)) wi.cin_timemin = time_min.getText();
        if(!x_max_b.isSelected() && !jScopeFacade.equalsString(x_max.getText(), wi.cin_xmax)) wi.cin_xmax = x_max.getText();
        if(!x_min_b.isSelected() && !jScopeFacade.equalsString(x_min.getText(), wi.cin_xmin)) wi.cin_xmin = x_min.getText();
        if(!upd_limits_b.isSelected() && (upd_limits.isSelected() != wi.cin_upd_limits)) wi.cin_upd_limits = upd_limits.isSelected();
        if(!y_max_b.isSelected() && !jScopeFacade.equalsString(y_max.getText(), wi.cin_ymax)) wi.cin_ymax = y_max.getText();
        if(!y_min_b.isSelected() && !jScopeFacade.equalsString(y_min.getText(), wi.cin_ymin)) wi.cin_ymin = y_min.getText();
        if(!title_b.isSelected() && !jScopeFacade.equalsString(title.getText(), wi.cin_title)) wi.cin_title = title.getText();
        if(!x_label_b.isSelected() && !jScopeFacade.equalsString(x_label.getText(), wi.cin_xlabel)) wi.cin_xlabel = x_label.getText();
        if(!y_label_b.isSelected() && !jScopeFacade.equalsString(y_label.getText(), wi.cin_ylabel)) wi.cin_ylabel = y_label.getText();
        // GAB 2014
    ...
    private int updateWI() {
        ...
        // wi.isContinuousUpdate = continuous_update_b.isSelected();
        wi.isContinuousUpdate = false;
        if(s[0].shot != UNDEF_SHOT) wi.shots = new long[num_signal];
        for(int i = 0; i < num_signal; i++){
            if(s[i].label != null) wi.in_label[i] = new String(s[i].label);
            if(s[i].x_expr != null) wi.in_x[i] = new String(s[i].x_expr);
            if(s[i].y_expr != null) wi.in_y[i] = new String(s[i].y_expr);
            if(wi.shots != null) wi.shots[i] = s[i].shot;
            if(!wi.is_image){
                ...
                if(s[i].up_err != null) wi.in_up_err[i] = new String(s[i].up_err);
                if(s[i].low_err != null) wi.in_low_err[i] = new String(s[i].low_err);
    ...
    private int checkSetup() {
        ...
        if(experiment.getText() == null || experiment.getText().trim().length() == 0) def_exp = false;
        if(shot.getText() == null || shot.getText().trim().length() == 0) def_shot = false;
        if(def_exp ^ def_shot){
            if(!def_shot){
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if(ob instanceof TextField){
            if(ob == x_max || ob == y_max || ob == x_min || ob == y_min || ob == shot || ob == time_max || ob == time_min){
                if(!Character.isDigit(key) && key != KeyEvent.VK_DELETE && key != '.' && key != '+' && key != '-') return;
            }
        }
    }

    private static void defaultButtonOperation(Object obj, boolean state, String val) {
    ...
    private void putShotValue(boolean def_flag) {
        if(def_flag) wi.defaults |= (1 << MdsWaveInterface.B_shot);
        else wi.defaults &= ~(1 << MdsWaveInterface.B_shot);
    ...
    private void DefaultButtonChange(Object ob) {
        boolean def_flag;
        if(ob == title_b) defaultButtonOperation(title, def_flag = title_b.isSelected(), wi.GetDefaultValue(MdsWaveInterface.B_title, def_flag));
        if(ob == shot_b) putShotValue(shot_b.isSelected());
        if(ob == experiment_b) defaultButtonOperation(experiment, def_flag = experiment_b.isSelected(), wi.GetDefaultValue(MdsWaveInterface.B_exp, def_flag));
        if(ob == upd_event_b) defaultButtonOperation(upd_event, def_flag = upd_event_b.isSelected(), wi.GetDefaultValue(MdsWaveInterface.B_event, def_flag));
        if(ob == def_node_b) defaultButtonOperation(def_node, def_flag = def_node_b.isSelected(), wi.GetDefaultValue(MdsWaveInterface.B_default_node, def_flag));
        if(ob == x_max_b) defaultButtonOperation(x_max, def_flag = x_max_b.isSelected(), wi.GetDefaultValue(MdsWaveInterface.B_x_max, def_flag));
        if(ob == x_min_b) defaultButtonOperation(x_min, def_flag = x_min_b.isSelected(), wi.GetDefaultValue(MdsWaveInterface.B_x_min, def_flag));
        if(ob == time_max_b) defaultButtonOperation(time_max, def_flag = time_max_b.isSelected(), wi.GetDefaultValue(MdsWaveInterface.B_x_max, def_flag));
        if(ob == time_min_b) defaultButtonOperation(time_min, def_flag = time_min_b.isSelected(), wi.GetDefaultValue(MdsWaveInterface.B_x_min, def_flag));
        if(ob == x_label_b) defaultButtonOperation(x_label, def_flag = x_label_b.isSelected(), wi.GetDefaultValue(MdsWaveInterface.B_x_label, def_flag));
        if(ob == y_max_b) defaultButtonOperation(y_max, def_flag = y_max_b.isSelected(), wi.GetDefaultValue(MdsWaveInterface.B_y_max, def_flag));
        if(ob == y_min_b) defaultButtonOperation(y_min, def_flag = y_min_b.isSelected(), wi.GetDefaultValue(MdsWaveInterface.B_y_min, def_flag));
        if(ob == y_label_b) defaultButtonOperation(y_label, def_flag = y_label_b.isSelected(), wi.GetDefaultValue(MdsWaveInterface.B_y_label, def_flag));
        if(ob == upd_limits_b) defaultButtonOperation(upd_limits, def_flag = upd_limits_b.isSelected(), wi.GetDefaultValue(MdsWaveInterface.B_update, def_flag));
    }
    ...
    class SymContainer extends java.awt.event.ContainerAdapter{
        ...
        public void componentAdded(java.awt.event.ContainerEvent event) {
            ...
            if(object == SetupDataDialog.this) SetupDataDialog_componentAdded(event);
        }
    }
