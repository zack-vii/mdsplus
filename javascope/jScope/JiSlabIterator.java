...
class JiSlabIterator{
    ...
    private int getOffset() throws IOException {
        return innerProduct(mProducts, mCounter) + mOffset;
    }

    private static int innerProduct(int[] v1, int[] v2) {
        int rval = 0;
    ...
    public JiSlab next() throws IOException {
        JiSlab rval = null;
        if(!mFinished){
            if(mIsRecord && mSizes.length == 1) rval = new JiSlab(getOffset(), 1);
            else rval = new JiSlab(getOffset(), mSizes[0]);
            if(!mIsRecord && mCounter.length == 1){
                mFinished = true;
            }else{ // Advance counter and ripple carry if needed
                int length = mSizes.length;
                int i;
                if(mIsRecord && length == 1) i = 0;
                else i = 1;
                for(; i < length; ++i){
                    if(DEBUG){
