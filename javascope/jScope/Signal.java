...
final public class Signal implements WaveDataListener{
        ...
        void addRegion(RegionDescriptor newReg) {
            ...
            if(currRegion.lowerBound < newReg.lowerBound){
                ...
                }else // The new region is completely contained in currRegion
                {
                    ...
                    idx++;
                    lowResRegions.insertElementAt(new RegionDescriptor(newReg.upperBound, prevUpper, currRegion.resolution), idx);
    ...
    public Signal(WaveData data, WaveData x_data, double xminVal, double xmaxVal, WaveData lowErrData, WaveData upErrData){
        ...
        try{
            ...
            if(saved_xmin == Double.NEGATIVE_INFINITY) saved_xmin = this.xmin;
            if(saved_xmax == Double.POSITIVE_INFINITY) saved_xmax = this.xmax;
    ...
    public Signal(float _x[], float _y[], int _n_points, double _xmin, double _xmax, double _ymin, double _ymax){
        ...
        xmin = _xmin;
        xmax = _xmax;
        if(xmax - xmin < _x[1] - _x[0]) xmax = xmin + _x[1] - _x[0];
        saved_xmin = curr_xmin = xmin;
        saved_xmax = curr_xmax = xmax;
        if(xmax <= xmin) saved_xmax = xmax = xmin + (float)1E-6;
        if(_ymin > _ymax) _ymin = _ymax;
        saved_ymin = ymin = _ymin;
        saved_ymax = ymax = _ymax;
        curr_xmax = xmax;
        curr_xmin = xmin;
        setAxis(); // Here xmin and xmax have been passed, so override values computed by setAxis()
        ymin = saved_ymin;
        ymax = saved_ymax;
        saved_xmin = curr_xmin = xmin;
        saved_xmax = curr_xmax = xmax;
    ...
    public Signal(Signal s){
        ...
        z2D_max = s.z2D_max;
        z2D_min = s.z2D_min;
        if(xmax <= xmin) saved_xmax = xmax = xmin + 1E-6;
    ...
    public Signal(Signal s, double start_x, double end_x, double start_y, double end_y){
        ...
        n_nans = s.n_nans;
        error = s.error;
        if(error) upError = s.upError;
        asym_error = s.asym_error;
        if(asym_error) lowError = s.lowError;
        increasing_x = s.increasing_x;
        saved_ymax = s.saved_ymax;
        ymax = end_y;
        saved_ymin = s.saved_ymin;
        ...
        xmax = end_x;
        if(xmax <= xmin) saved_xmax = curr_xmax = xmax = xmin + 1E-6;
        marker = s.marker;
        marker_step = s.marker_step;
        color_idx = s.color_idx;
        color = s.color;
    ...
    public double getX(int idx) {
        if(this.type == Signal.TYPE_2D && (mode2D == Signal.MODE_YZ || mode2D == Signal.MODE_XZ)) return sliceX[idx];
        try{
            return x[idx];
        }catch(Exception exc){
            return 0;
    ...
    public float getY(int idx) {
        if(this.type == Signal.TYPE_2D && (mode2D == Signal.MODE_YZ || mode2D == Signal.MODE_XZ)) return sliceY[idx];
        try{
            return y[idx];
        }catch(Exception exc){
            return 0;
    ...
    public float getZ(int idx) {
        if(z == null) z = data.getZ();
        return z[idx];
    }

    public double[] getX() throws Exception {
        if(type == TYPE_2D && (mode2D == MODE_XZ || mode2D == MODE_YZ)) return sliceX;
        return x;
    }

    public float[] getY() throws Exception {
        if(type == TYPE_2D && (mode2D == MODE_XZ || mode2D == MODE_YZ)) return sliceY;
        return y;
    }
    ...
    public double[] getX2D() {
        if(x2D == null) x2D = data.getX2D();
        return x2D;
    }

    public float[] getY2D() {
        if(y2D == null) y2D = data.getY2D();
        return y2D;
    }
    ...
    public String getStringOfXinYZplot() {
        if(this.isLongX()) return toStringTime((long)curr_x_yz_plot);
    ...
    public int getNumPoints() {
        if(this.type == Signal.TYPE_2D && (mode2D == Signal.MODE_YZ || mode2D == Signal.MODE_XZ)) return sliceX.length;
        if(data != null){
            try{
        ...
        if(this.type == Signal.TYPE_2D){
            switch(mode2D){
                case Signal.MODE_IMAGE:
                    ...
                    if(z != null && idx < z.length){ return z[idx]; }
    ...
    private static int getArrayIndex(double arr[], double d) {
        int i = -1;
        if(i == -1){
    ...
    private static int getArrayIndex(float arr[], double d) {
        int i = -1;
        if(i == -1){
            for(i = 0; i < arr.length - 1; i++){
                if((d > arr[i] && d < arr[i + 1]) || d == arr[i]) break;
    ...
    public void showYZ(double t) {
        if(curr_x_yz_plot == t && mode2D == MODE_YZ) return;
    ...
    public void showYZ(int idx) {
        float[] y2d = y2D;
        double[] x2d = x2D;
        if((idx >= x2d.length || idx == curr_x_yz_idx) && mode2D == MODE_YZ) return;
        prev_idx = 0;
        ...
        if(zY2D != null){
            ...
            for(int j = 0; j < y2d.length; j++){
                ...
                y2d[j] = zY2D[k];
                if(ymin > y2d[j]) ymin = y2d[j];
                if(ymax < y2d[j]) ymax = y2d[j];
            }
        }
        ...
        for(int j = 0; j < y2d.length; j++){
            ...
            if(k >= zLen) break;
            sliceY[j] = z[k];
            if(sliceMin > z[k]) sliceMin = z[k];
            if(sliceMax < z[k]) sliceMax = z[k];
        }
        error = asym_error = false;
        mode2D = Signal.MODE_YZ;
        if(!fix_xmin) saved_xmin = curr_xmin = xmin = y2D_min;
        // saved_xmin = curr_xmin = ymin;
        if(!fix_xmax) saved_xmax = curr_xmax = xmax = y2D_max;
        // saved_xmax = curr_xmax = ymax;
        if(!fix_ymin) saved_ymin = ymin = sliceMin;
        if(!fix_ymax) saved_ymax = ymax = sliceMax;
    ...
    public void decShowYZ() {
        if(type == TYPE_2D && mode2D == Signal.MODE_YZ){
            ...
            if(idx < 0) idx = x2D.length - 1;
            showYZ(idx);
        }
    }

    public void showXZ(double xd) {
        if(curr_y_xz_plot == xd) return;
    ...
    public void showXZ(int idx) {
        ...
        if((idx >= y2d.length || idx == curr_y_xz_idx) && mode2D == MODE_XZ) return;
        prev_idx = 0;
        ...
        if(zY2D != null){
            ...
            for(int j = 0; j < x2D.length; j++){
                ...
                else if(x2d[j] < curr_xmin) curr_xmin = x2d[j];
            }
        }
        ...
        if(!fix_xmin) saved_xmin = curr_xmin = xmin = x2D_min;
        // saved_xmin = curr_xmin;
        if(!fix_xmax) saved_xmax = curr_xmax = xmax = x2D_max;
    ...
    public void setMode2D(int mode) {
        ...
        switch(mode){
            ...
            case MODE_YZ:
                double v = x2D[0];
                if(!Double.isNaN(curr_x_yz_plot)) v = curr_x_yz_plot;
                setMode2D(mode, v);
                break;
            case MODE_CONTOUR:
                setMode2D(mode, 0);
    ...
    public void setMode2D(int mode, double value) {
        ...
        switch(mode){
            case MODE_IMAGE:
                 ...
                 */
                if(saved_ymin == Double.NEGATIVE_INFINITY) saved_ymin = ymin = y2D_min;
                else ymin = saved_ymin;
                if(saved_ymax == Double.POSITIVE_INFINITY) saved_ymax = ymax = y2D_max;
                else ymax = saved_ymax;
                if(saved_xmin == Double.NEGATIVE_INFINITY) saved_xmin = xmin = x2D_min;
                else xmin = saved_xmin;
                if(saved_xmax == Double.POSITIVE_INFINITY) saved_xmax = xmax = x2D_max;
                else xmax = saved_xmax;
                break;
            case MODE_XZ:
                showXZ((float)value);
                break;
    ...
    public void setAttributes(String name, int color_idx, int marker, int marker_step, boolean interpolate) {
        setMarker(marker);
        setMarkerStep(marker_step);
        setInterpolate(interpolate);
        setColorIdx(color_idx);
        setName(name);
    }
    ...
    public void setMarkerStep(int value) {
        marker_step = value;
    }
    ...
    public void setColorIdx(int value) {
        color = null;
        color_idx = value;
    }
    ...
    public void setName(String value) {
        if(value != null && value.length() != 0) name = new String(value);
    }
    ...
    public int getMarkerStep() {
        return (marker == POINT) ? 1 : marker_step;
    }
    ...
    public void AutoscaleX() {
        ...
        double currX[];
        if(type == Signal.TYPE_2D && (mode2D == MODE_XZ || mode2D == MODE_YZ)) currX = sliceX;
        else currX = x;
        if(x == null || x.length == 0) return;
        xmin = xmax = currX[0];
        for(int i = 1; i < currX.length; i++){
            if(currX[i] < xmin) xmin = currX[i];
            if(currX[i] > xmax) xmax = currX[i];
        }
        if(xmin == xmax) xmax = xmin + (float)1E-10;
    }
    ...
    public void AutoscaleY() {
        ...
        if(type == Signal.TYPE_2D){
            if(mode2D == Signal.MODE_IMAGE || mode2D == Signal.MODE_CONTOUR){
                ...
                for(int i = 0; i < sliceY.length; i++){
                    if(sliceY[i] < ymin) ymin = sliceY[i];
                    if(sliceY[i] > ymax) ymax = sliceY[i];
                }
                if(ymin == ymax) ymax = ymin == 0 ? 1 : (ymin < 0 ? ymin - ymin / 4 : ymin + ymin / 4);
            }
            return;
        }
        float currY[];
        if(type == Signal.TYPE_2D && (mode2D == MODE_XZ || mode2D == MODE_YZ)) currY = sliceY;
        else currY = y;
        int startIdx;
        ...
        for(int i = startIdx; i < currY.length; i++){
            ...
            if(currY[i] < ymin) ymin = currY[i];
            if(currY[i] > ymax) ymax = currY[i];
        }
        if(ymin == ymax) ymax = ymin + ymin / 4;
    }
    ...
    void checkData(double xMin, double xMax) throws Exception {
        ...
        if(numDimensions == 1){
            ...
            {
                XYData xyData;
                if(!error) xyData = data.getData(xMin, xMax, NUM_POINTS);
                else xyData = data.getData(xMin, xMax, Integer.MAX_VALUE);
                if(xyData == null) return; // empty signal
                x = xyData.x;
                y = xyData.y;
                adjustArraySizes();
                increasing_x = xyData.increasingX;
                if(xMin == Double.NEGATIVE_INFINITY) this.xmin = curr_xmin = xyData.xMin;
                else this.xmin = curr_xmin = xMin;
                if(xMax == Double.POSITIVE_INFINITY) this.xmax = curr_xmax = xyData.xMax;
                else this.xmax = curr_xmax = xMax;
                this.ymin = this.ymax = y[0];
                for(int i = 0; i < y.length; i++){
                    if(y[i] < this.ymin) this.ymin = y[i];
                    if(y[i] > this.ymax) this.ymax = y[i];
                }
                if(data.isXLong()) xLong = xyData.xLong;
            ...
            if(saved_ymin == Double.NEGATIVE_INFINITY) saved_ymin = ymin;
            if(saved_ymax == Double.POSITIVE_INFINITY) saved_ymax = ymax;
            ...
            if(x_data != null){
                ...
                yY2D = x_data.getY2D();
                zY2D = x_data.getZ();
                if((x2D != null && x2D.length != xY2D.length) || (x2DLong != null && x2DLong.length != xY2D.length) && y2D.length != yY2D.length && z.length != zY2D.length){
                    xY2D = null;
                    yY2D = null;
                    zY2D = null;
                    x_data = null;
            ...
            for(int i = 0; i < x2D.length; i++){
                if(x2DVal[i] < x2D_min) x2D_min = x2DVal[i];
                if(x2DVal[i] > x2D_max) x2D_max = x2DVal[i];
            }
            y2D_min = y2D_max = y2D[0];
            for(int i = 0; i < y2D.length; i++){
                if(y2D[i] < y2D_min) y2D_min = y2D[i];
                if(y2D[i] > y2D_max) y2D_max = y2D[i];
            }
            z2D_min = z2D_max = z[0];
            for(int i = 0; i < z.length; i++){
                if(z[i] < z2D_min) z2D_min = z[i];
                if(z[i] > z2D_max) z2D_max = z[i];
            }
            if(xMin == Double.NEGATIVE_INFINITY) this.xmin = curr_xmin = x2D_min;
            else this.xmin = curr_xmin = xMin;
            if(xMax == Double.POSITIVE_INFINITY) this.xmax = curr_xmax = x2D_max;
            else this.xmax = curr_xmax = xMax;
        }
    }
    ...
    private static int FindIndex(float d[], double v, int pIdx) {
    ...
    private static int FindIndex(double d[], double v, int pIdx) {
        int i;
        if(v > d[pIdx]){
    ...
    public int FindClosestIdx(double curr_x, double curr_y) {
        ...
        if(this.type == Signal.TYPE_2D && (mode2D == Signal.MODE_IMAGE || mode2D == Signal.MODE_CONTOUR)){
            ...
            if(img_xprev > y2D.length) return img_xprev - 6;
            return img_xprev;
        }
        double currX[];
        if(type == Signal.TYPE_1D) currX = x;
        else{
            if(mode2D == MODE_XZ || mode2D == MODE_YZ) currX = sliceX;
            else{
                double xf[] = x2D;
        ...
        if(increasing_x || type == Signal.TYPE_2D){
            ...
            if(prev_idx >= currX.length) prev_idx = currX.length - 1;
        ...
        for(i = 0; i < x.length - 1; i++){
            ...
            if(curr_x > currX[i] && curr_x < currX[i + 1] || curr_x < currX[i] && curr_x > currX[i + 1] || currX[i] == currX[i + 1]){
                ...
                if(currX[i] != currX[i + 1] && !Float.isNaN(y[i + 1])) curr_dist += (curr_x - currX[i + 1]) * (curr_x - currX[i + 1]) + (curr_y - y[i + 1]) * (curr_y - y[i + 1]);
    ...
    public boolean isLongX() {
        return((type == TYPE_1D || type == TYPE_2D && (mode2D == Signal.MODE_XZ || mode2D == Signal.MODE_IMAGE)) && xLong != null);
    }

    public boolean isLongXForLabel() {
        return (type == TYPE_1D || type == TYPE_2D && (mode2D == Signal.MODE_XZ || mode2D == Signal.MODE_YZ || mode2D == Signal.MODE_IMAGE)) && data.isXLong();
    }
    ...
    public void setXLimits(double xmin, double xmax, int mode) {
        ...
        if(xmin != Double.NEGATIVE_INFINITY){
            ...
            if((mode & FIXED_LIMIT) != 0) fix_xmin = true;
        }
        if(xmax != Double.POSITIVE_INFINITY){
            this.xmax = xmax;
            if((mode & AT_CREATION) != 0) this.saved_xmax = xmax;
            if((mode & FIXED_LIMIT) != 0) fix_xmax = true;
        }
        double actXMin = xmin;
        ...
        if(!increasing_x) return; // Dynamic resampling only for "classical" signas
        if(up_errorData != null || low_errorData != null) return; // Dynamic resampling only without error bars
        ...
        for(int i = 0; i < lowResRegions.size(); i++){
             ...
             */
            if(((mode & DO_NOT_UPDATE) == 0) && (currLower != saved_xmin || currUpper != saved_xmax || (mode & AT_CREATION) == 0)) data.getDataAsync(currLower, currUpper, NUM_POINTS);
        }
        // fireSignalUpdated();
    }
    ...
    public void setYmin(double ymin, int mode) {
        ...
        this.ymin = ymin;
        if((mode & AT_CREATION) != 0) this.saved_ymin = ymin;
        if((mode & FIXED_LIMIT) != 0) fix_ymin = true;
    }

    public void setYmax(double ymax, int mode) {
        if(ymax == Double.POSITIVE_INFINITY) return;
        this.ymax = ymax;
        if((mode & AT_CREATION) != 0) this.saved_ymax = ymax;
        if((mode & FIXED_LIMIT) != 0) fix_ymax = true;
    }
    ...
    void setAxis(double x2D[], float z2D[], float y2D[], int xIdx, int zIdx, int yIdx) {
        ...
        for(i = xIdx; i < x2D.length; i++){
            if(x2D[i] > x2D_max) x2D_max = x2D[i];
            if(x2D_min > x2D[i]) x2D_min = x2D[i];
        }
        for(i = zIdx; i < z2D.length; i++){
            if(z2D[i] > z2D_max) z2D_max = z2D[i];
            if(z2D_min > z2D[i]) z2D_min = z2D[i];
        }
        for(i = yIdx; i < y2D.length; i++){
            if(y2D[i] > y2D_max) y2D_max = y2D[i];
            if(y2D_min > y2D[i]) y2D_min = y2D[i];
        }
    }
    ...
    void setAxis() {
        ...
        if(type != TYPE_1D || !increasing_x) return;
        ...
        try{
            ...
            increasing_x = xyData.increasingX;
            if(increasing_x){
                resolutionManager.addRegion(new RegionDescriptor(Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, NUM_POINTS / (x[x.length - 1] - x[0])));
            }
            if(data.isXLong()) xLong = xyData.xLong;
            ...
            for(i = 0; i < x.length; i++){
                if(Float.isNaN(y[i]) && n_nans < 100) nans[n_nans++] = i;
    ...
    public void setStartIndexToUpdate() {
        if(x != null) startIndexToUpdate = x.length;
    }
    ...
    private static double[] appendArray(double arr1[], int sizeUsed, double arr2[], int incSize) {
        /*
    ...
    private static float[] appendArray(float arr1[], int sizeUsed, float arr2[], int incSize) {
        if(arr1 == null) return arr2.clone();
        if(arr2 == null) return arr1.clone();
        float val[];
        if(arr1.length < sizeUsed + arr2.length){
    ...
    public void dataRegionUpdated(double[] regX, float[] regY, double resolution) {
        ...
        for(samplesAfter = 0; samplesAfter < x.length - 1 && x[x.length - samplesAfter - 1] > regX[regX.length - 1]; samplesAfter++);
        ...
        if(x.length == 0 || regX[0] >= x[x.length - 1]) // Data are being appended
        {
            ...
            if(xmax < newX[newX.length - 1]) xmax = newX[newX.length - 1];
            x = newX;
            y = newY;
    ...
    public void sourceUpdated(XYData xydata) {
        ...
        if(this.xlabel == null) try{
            this.xlabel = (x_data == null) ? data.GetXLabel() : x_data.GetYLabel();
        }catch(Exception e){}
        if(this.ylabel == null) try{
            this.ylabel = data.GetYLabel();
        }catch(Exception e){}
        if(this.zlabel == null && type == TYPE_2D) try{
            this.zlabel = data.GetZLabel();
        }catch(Exception e){}
        try{
            x = xydata.x;
            xLong = xydata.xLong;
    ...
    public void dataRegionUpdated(long[] regX, float[] regY, double resolution) {
        ...
        }else // Data Appended
        {
            ...
            if(samplesBefore > 0 && samplesBefore < xLong.length && xLong[samplesBefore] > regX[0]) samplesBefore--;
            for(samplesAfter = 0; samplesAfter < xLong.length - 1 && xLong[xLong.length - samplesAfter - 1] > regX[regX.length - 1]; samplesAfter++);
            if(samplesAfter > 0 && xLong.length - samplesAfter - 1 >= 0 && xLong[xLong.length - samplesAfter - 1] < regX[regX.length - 1]) samplesAfter--;
            double[] newX = new double[samplesBefore + regX.length + samplesAfter];
            ...
            if(regX[0] >= xLong[xLong.length - 1]) // Data are being appended
            {
                ...
                if(freezeMode == FREEZED_SCROLL){
                    xmax += delta;
                    xmin += delta;
                }else if(freezeMode == NOT_FREEZED) xmax = newX[newX.length - 1];
                x = newX;
                xLong = newXLong;
                y = newY;
                fireSignalUpdated(true);
    ...
    void freeze() {
        if(isLongX() && xmax > xLong[xLong.length - 1]) freezeMode = FREEZED_SCROLL;
        else freezeMode = FREEZED_BLOCK;
        freezedXMin = xmin;
        freezedXMax = xmax;
    }
    ...
    void unfreeze() {
        ...
        for(int i = 0; i < pendingUpdatesV.size(); i++)
            if(pendingUpdatesV.elementAt(i).xLong != null) dataRegionUpdated(pendingUpdatesV.elementAt(i).xLong, pendingUpdatesV.elementAt(i).y, pendingUpdatesV.elementAt(i).resolution);
            else dataRegionUpdated(pendingUpdatesV.elementAt(i).x, pendingUpdatesV.elementAt(i).y, pendingUpdatesV.elementAt(i).resolution);
    ...
    void fireSignalUpdated(boolean changeLimits) {
        if(DEBUG.M){
            System.out.println("fireSignalUpdated(" + changeLimits + ") for " + signalListeners.size());
        }
