...
final public class SetupDefaults extends JDialog implements ActionListener{
    ...
    private JComboBox   grid_mode, legend_mode, auto_color_mode;
    private JCheckBox   reversed_b, upd_limits;
    ...
    private static int IsGridMode(String mode) {
    ...
    private static void setTextValue(JTextField t, String val) {
        if(val != null){
            t.setText(val);
        }
    }
    ...
    public void SaveDefaultConfiguration(jScopeDefaultValues def_vals) {
        ...
        if(auto_color_mode.getSelectedIndex() == 0) WaveInterface.auto_color_on_expr = false;
        ...
        if(x_curr_lines_grid > Grid.MAX_GRID) x_curr_lines_grid = Grid.MAX_GRID;
        x_grid_lines.setText("" + x_curr_lines_grid);
        try{
        ...
        if(y_curr_lines_grid > Grid.MAX_GRID) y_curr_lines_grid = Grid.MAX_GRID;
        y_grid_lines.setText("" + y_curr_lines_grid);
    ...
    public boolean IsChanged(jScopeDefaultValues def_vals) {
        if(!jScopeFacade.equalsString(shot.getText(), def_vals.shot_str)) return true;
        if(!jScopeFacade.equalsString(experiment.getText(), def_vals.experiment_str)) return true;
        if(!jScopeFacade.equalsString(upd_event.getText(), def_vals.upd_event_str)) return true;
        if(!jScopeFacade.equalsString(def_node.getText(), def_vals.def_node_str)) return true;
        if(!jScopeFacade.equalsString(title.getText(), def_vals.title_str)) return true;
        if(!jScopeFacade.equalsString(x_max.getText(), def_vals.xmax)) return true;
        if(!jScopeFacade.equalsString(x_min.getText(), def_vals.xmin)) return true;
        if(!jScopeFacade.equalsString(x_label.getText(), def_vals.xlabel)) return true;
        if(!jScopeFacade.equalsString(y_max.getText(), def_vals.ymax)) return true;
        if(!jScopeFacade.equalsString(y_min.getText(), def_vals.ymin)) return true;
        if(!jScopeFacade.equalsString(y_label.getText(), def_vals.ylabel)) return true;
