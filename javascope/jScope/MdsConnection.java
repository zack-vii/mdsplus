...
public class MdsConnection{
    ...
    class PMET extends Thread // Process Mds Event Thread
    {
        ...
        public void run() {
            ...
            if(eventName != null) dispatchUpdateEvent(eventName);
            else if(eventId != -1) dispatchUpdateEvent(eventId);
        }
    ...
    */
    ...
    public void setUser(String user) {
        if(user == null || user.length() == 0) this.user = DEFAULT_USER;
        else this.user = user;
    }
    ...
    public synchronized String getProviderHost() {
        ...
        if(idx_1 == -1) idx_1 = provider.length();
        if(idx != -1) address = provider.substring(idx + 1, idx_1);
        else address = provider.substring(0, idx_1);
        return address;
    }
    ...
    public synchronized int getProviderPort() throws NumberFormatException {
        ...
        int idx = provider.indexOf(":");
        if(idx != -1) port = Integer.parseInt(provider.substring(idx + 1, provider.length()));
        return port;
    }
    ...
    public synchronized Descriptor MdsValue(String expr, Vector<Descriptor> args, boolean wait) {
        ...
        try{
            ...
            if(wait){
                out = getAnswer();
                if(out == null) out = new Descriptor("Could not get IO for " + provider);
    ...
    public synchronized Descriptor MdsValue(String expr) {
        ...
        try{
            ...
            return new Descriptor("Could not get IO for " + provider + exc);
        }
    }

    public int DisconnectFromMds() {
        try{
            if(connection_listener.size() > 0) connection_listener.removeAllElements();
            dos.close();
            dis.close();
    ...
    public synchronized int RemoveEvent(UpdateEventListener l, String eventName) {
        int eventid = -1;
        if(hashEventName.containsKey(eventName)){
            EventItem eventItem = hashEventName.get(eventName);
    ...
    public synchronized void dispatchUpdateEvent(int eventid) {
        if(hashEventId.containsKey(eventid)) dispatchUpdateEvent(hashEventId.get(eventid));
    }

    public synchronized void dispatchUpdateEvent(String eventName) {
        if(hashEventName.containsKey(eventName)) dispatchUpdateEvent(hashEventName.get(eventName));
    }
    ...
    public synchronized void MdsSetEvent(UpdateEventListener l, String event) {
        int eventid;
        if((eventid = AddEvent(l, event)) == -1) return;
        try{
            ...
            sendArg((byte)2, Descriptor.DTYPE_UBYTE, (byte)3, null, new byte[]{(byte)(eventid)});
        }catch(IOException e){
    ...
    public synchronized void MdsRemoveEvent(UpdateEventListener l, String event) {
        int eventid;
        if((eventid = RemoveEvent(l, event)) == -1) return;
        try{
            sendArg((byte)0, Descriptor.DTYPE_CSTRING, (byte)2, null, MdsMessage.EVENTCANREQUEST.getBytes());
            sendArg((byte)1, Descriptor.DTYPE_CSTRING, (byte)2, null, new byte[]{(byte)eventid});
        }catch(IOException e){
            error = new String("Could not get IO for " + provider + e);
        }
    }
