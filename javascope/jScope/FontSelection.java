...
public class FontSelection extends JDialog implements ActionListener, ItemListener{
    ...
    public FontSelection(Frame dw, String title){
        ...
        sizePanel.add(sizes);
        style_l = new String[]{"PLAIN", "BOLD", "ITALIC", "BOLD & ITALIC"};
    ...
    private void GetPropertiesValue() {
        ...
        if(prop == null) font = StringToFont(getFont().toString());
        else font = StringToFont(prop);
        if(font != null){
            setFontChoice();
    ...
    public Font StringToFont(String f) {
        ...
        for(i = 0; i < this.style_l.length; i++)
            if(style_l[i].equals(style.toUpperCase())) break;
        if(i == this.style_l.length) stChoice = 0;
        else stChoice = i;
    ...
    public void toFile(PrintWriter out, String prompt) {
        if(font != null) out.println(prompt + ": " + font.toString());
        out.println("");
    }
