...
public class MultiWaveform extends Waveform{
    ...
    public void Copy(Waveform wave) {
        ...
        if(!wave.is_image){
            ...
            for(i = 0; i < s.size(); i++){
                ...
                if(DEBUG.D){
                    System.out.println("Copy: " + signals.elementAt(i));
                }
            ...
            UpdateLimits();
            if(waveform_signal != null) super.Update(waveform_signal);
        }
    }

    public void removeSignal(int idx) {
        if(idx < signals.size()) signals.removeElementAt(idx);
    }
    ...
    public void addSignals(Signal s[]) {
        ...
        for(int i = 0; i < s.length; i++){
            addSignal(s[i]);
            if(s[i] != null) s[i].registerSignalListener(this);
        }
        setLimits();
    ...
    public void setLegendMode(int legend_mode) {
        ...
        if(legend_mode != MultiWaveform.LEGEND_IN_GRAPHICS) fixed_legend = true;
        else fixed_legend = false;
    }
    ...
    protected void PaintSignal(Graphics g, Dimension d, int print_mode) {
        Dimension dim;
        if(print_mode == NO_PRINT) dim = getWaveSize();
        else dim = getPrintWaveSize(d);
        ...
        if(fixed_legend && show_legend || (print_mode & PRINT_LEGEND) == PRINT_LEGEND){
            ...
            if(legend_mode == LEGEND_BOTTOM && bottom_size != 0) drawLegend(g, new Point(0, dim.height), print_mode, HORIZONTAL);
            if(legend_mode == LEGEND_RIGHT && right_size != 0) drawLegend(g, new Point(dim.width, 0), print_mode, VERTICAL);
        }
    }
    ...
    public synchronized void UpdateSignals(Signal signals[], int timestamp, boolean force_update) {
        ...
        Signal s;
        if(!force_update && update_timestamp != timestamp) return;
        ...
        if(waveform_signal != null){
            if(curr_point_sig_idx > signals.length || curr_point_sig_idx == -1) curr_point_sig_idx = 0;
            if(force_update && continuosAutoscale) super.Update(waveform_signal);
            else super.UpdateSignal(waveform_signal);
        }
        if(asinchAutoscale){
            Autoscale();
            asinchAutoscale = false;
        }
        if(force_update) SetEnabledDispatchEvents(true);
    }
    ...
    public String getSignalName(int idx) {
        if(idx < signals.size() && signals.elementAt(idx) != null){
            ...
            return s.getName();
        }else if(is_image && frames != null) return frames.getName();
        return null;
    }
    ...
    public boolean GetInterpolate(int idx) {
        if(idx < signals.size()){ return signals.elementAt(idx).getInterpolate(); }
        return false;
    }
    ...
    public int GetMarker(int idx) {
        if(idx < signals.size()){ return signals.elementAt(idx).getMarker(); }
        return 0;
    }
    ...
    public int GetMarkerStep(int idx) {
        if(idx < signals.size()){ return signals.elementAt(idx).getMarkerStep(); }
        return 0;
    }
    ...
    public void SetColorIdx(int idx, int color_idx) {
        ...
        if(idx < signals.size()){
            ...
            if(idx == curr_point_sig_idx) crosshair_color = colors[color_idx % colors.length];
        }
    }

    public int GetColorIdx(int idx) {
        if(is_image) return super.GetColorIdx();
        if(idx < signals.size()){ return signals.elementAt(idx).getColorIdx(); }
        return 0;
    }

    @Override
    public int GetShowSignalCount() {
        if(signals != null) return signals.size();
        return 0;
    }
    ...
    public void setPointSignalIndex(int idx) {
        if(idx >= 0 && idx < signals.size()){
            ...
            if(curr_signal.getColor() != null) crosshair_color = curr_signal.getColor();
            else crosshair_color = colors[curr_signal.getColorIdx() % colors.length];
        }
    }
    ...
    public void SetSignalState(String label, boolean state) {
        ...
        if(signals != null){
            ...
            if(mode == Waveform.MODE_POINT){
                Dimension d = getSize();
                double curr_x = wm.XValue(end_x, d), curr_y = wm.YValue(end_y, d);
    ...
    void UpdateLimits() {
        ...
        if(curr_point_sig_idx == -1 || curr_point_sig_idx >= signals.size() || signals.elementAt(curr_point_sig_idx) == null){
            for(i = 0; i < signals.size(); i++)
                if(signals.elementAt(i) != null) break;
            if(i == signals.size()) return;
        }else i = curr_point_sig_idx;
        waveform_signal = new Signal(signals.elementAt(i));
        if(DEBUG.D){
            System.out.println("UpdateLimits: " + waveform_signal);
        }
        ...
        for(i = 0; i < signals.size(); i++){
            if(signals.elementAt(i) == null) continue;
        ...
        for(i = 0; i < signals.size(); i++){
            ...
            if(anyLongX && !signals.elementAt(i).isLongX()) continue;
            if(waveform_signal.getXmax() < signals.elementAt(i).getXmax()) waveform_signal.setXLimits(waveform_signal.getXmin(), signals.elementAt(i).getXmax(), Signal.AT_CREATION);
            if(waveform_signal.getXmin() > signals.elementAt(i).getXmin()) waveform_signal.setXLimits(signals.elementAt(i).getXmin(), waveform_signal.getXmax(), Signal.AT_CREATION);
            if(waveform_signal.getYmax() < signals.elementAt(i).getYmax()) waveform_signal.setYmax(signals.elementAt(i).getYmax(), Signal.AT_CREATION);
            if(waveform_signal.getYmin() > signals.elementAt(i).getYmin()) waveform_signal.setYmin(signals.elementAt(i).getYmin(), Signal.AT_CREATION);
        }
    }
    ...
    protected void drawLegend(Graphics g, Point p, int print_mode, int orientation) {
        ...
        if(orientation == VERTICAL) g.translate(-marker_width, 0);
        for(int i = 0, py = p.y + h, px = p.x; i < getSignalCount(); i++){
            if(!isSignalShow(i)) continue;
            if((print_mode & PRINT_BW) != PRINT_BW) g.setColor(getSignalColor(i));
            s = getSignalInfo(i);
            if(orientation == HORIZONTAL){
    ...
    protected void drawSignal(Graphics g, Dimension d, int print_mode) {
        ...
        for(i = 0; i < signals.size(); i++){
            ...
            if(mode == MODE_PAN && dragging && s.getNumPoints() > MAX_DRAG_POINT) // dragging large signals
            {
                ...
                if(signals.size() == 1) drag_point = (s.getNumPoints() > MAX_DRAG_POINT * 3) ? MAX_DRAG_POINT * 3 : s.getNumPoints();
                ...
                if(s.getType() == Signal.TYPE_2D && (s.getMode2D() == Signal.MODE_IMAGE || s.getMode2D() == Signal.MODE_CONTOUR)){
                    ...
                    if(segments != null && (s.getInterpolate() || mode == MODE_PAN && dragging)) for(int k = 0; k < segments.size(); k++){
                        curr_polygon = segments.elementAt(k);
                        g.drawPolyline(curr_polygon.xpoints, curr_polygon.ypoints, curr_polygon.npoints);
                    }
                }
            }
            if(dragging && mode == MODE_PAN) continue;
    ...
    protected Point FindPoint(double curr_x, double curr_y, Dimension d, boolean is_first) {
        ...
        if(signals == null || signals.size() == 0) return null;
        // if(signals[curr_point_sig_idx] == null) return 0;
        if(!is_first) return FindPoint(signals.elementAt(curr_point_sig_idx), curr_x, curr_y, d);
        ...
        try{
            if(img_idx != -1){
                ...
                }else curr_point_sig_idx = img_idx;
            }
        }catch(Exception exc){}
    ...
    public synchronized void UpdatePoint(double curr_x, double curr_y) {
        if(!is_image){
            ...
            if(mode != MODE_POINT || signals == null || signals.size() == 0) return;
            Signal s;
            for(int i = 0; i < signals.size(); i++){
                s = signals.elementAt(i);
                if(s == null) continue;
    ...
    public Signal GetSignal() {
        if(signals != null && signals.size() > 0) return signals.elementAt(curr_point_sig_idx);
        return null;
    }

    public int getSignalMode1D(int idx) {
        if(idx >= 0 && idx < signals.size()) return signals.elementAt(idx).getMode1D();
        return -1;
    }

    public int getSignalMode2D(int idx) {
        if(idx >= 0 && idx < signals.size()) return signals.elementAt(idx).getMode2D();
        return -1;
    }
    ...
    */
    public int getSignalType(int idx) {
        if((idx >= 0 && idx < signals.size()) && (signals.elementAt(idx) != null)) return signals.elementAt(idx).getType();
        return -1;
    }
    ...
    protected void ReportLimits(ZoomRegion r, boolean add_undo) {
        ...
        if(add_undo) NotifyZoom(r.start_xs, r.end_xs, r.start_ys, r.end_ys, update_timestamp);
    }
    ...
    public void Autoscale() {
        ...
        for(i = 0; i < signals.size(); i++){
            ...
            v = signals.elementAt(i).getXmin();
            if(v < xmin) xmin = v;
            v = signals.elementAt(i).getXmax();
            if(v > xmax) xmax = v;
            v = signals.elementAt(i).getYmin();
            if(v < ymin) ymin = v;
            v = signals.elementAt(i).getYmax();
            if(v > ymax) ymax = v;
        }
    ...
    public void AutoscaleY() {
        if(waveform_signal == null || signals == null) return;
    ...
    public void SetXScale(Waveform w) {
        ...
        for(int i = 0; i < signals.size(); i++){
            if(signals.elementAt(i) == null) continue;
            signals.elementAt(i).setXLimits(w.waveform_signal.getXmin(), w.waveform_signal.getXmax(), Signal.SIMPLE);
        }
        ReportChanges();
    }
    ...
    public void SetXScaleAutoY(Waveform w) {
        ...
        if(w != this && orig_signals != null) // Previous zoom for differentr windows
        {
            ...
            orig_signals = new Vector<Signal>();
            for(int i = 0; i < signals.size(); i++)
    ...
    public void ResetScales() {
        if(signals == null || waveform_signal == null) return;
        if(orig_signals != null){
            signals = orig_signals;
            for(int i = 0; i < signals.size(); i++)
                if(signals.elementAt(i) != null) signals.elementAt(i).ResetScales();
        }
    ...
    public void signalUpdated(boolean changeLimits) {
        ...
        for(int i = 1; i < signals.size(); i++){
            ...
            if(signals.elementAt(i).isLongX()){
                anyLongX = true;
                waveform_signal.setXLimits(signals.elementAt(i).getXmin(), signals.elementAt(i).getXmax(), Signal.AT_CREATION);
                break;
            }
        }
        for(int i = 1; i < signals.size(); i++){
            ...
            // Avoid introducing 0 as limits for absolute times
            if(anyLongX && !(signals.elementAt(i)).isLongX()) continue;
            if(waveform_signal.getXmax() < signals.elementAt(i).getXmax()) waveform_signal.setXLimits(waveform_signal.getXmin(), signals.elementAt(i).getXmax(), Signal.SIMPLE);
            if(waveform_signal.getXmin() > signals.elementAt(i).getXmin()) waveform_signal.setXLimits(signals.elementAt(i).getXmin(), waveform_signal.getXmax(), Signal.SIMPLE);
            if(waveform_signal.getYmax() < signals.elementAt(i).getYmax()) waveform_signal.setYmax(signals.elementAt(i).getYmax(), Signal.SIMPLE);
            if(waveform_signal.getYmin() > signals.elementAt(i).getYmin()) waveform_signal.setYmin(signals.elementAt(i).getYmin(), Signal.SIMPLE);
        }
        repaint();
    }
}
