...
public class DeviceSetup extends JDialog{
    ...
    static DeviceSetup getDevice(final int baseNid) {
    ...
    public DeviceSetup(final boolean readOnly){
    ...
    public DeviceSetup(final JFrame f){
        this(f, false);
        DeviceSetup.openDevicesV.addElement(this);
    }

    public DeviceSetup(final JFrame f, final boolean readOnly){
        super(f);
    ...
    void addDeviceCloseListener(final DeviceCloseListener listener) {
    ...
    public void apply() {
        NidData oldNid = null;
        try{
            oldNid = this.subtree.getDefault(Tree.context);
            this.subtree.setDefault(new NidData(this.baseNid), Tree.context);
        }catch(final Exception exc){}
        ...
        try{
            this.subtree.setDefault(oldNid, Tree.context);
        }catch(final Exception exc){}
    ...
    public void apply(final int currBaseNid) {
        ...
        try{
            oldNid = this.subtree.getDefault(Tree.context);
            this.subtree.setDefault(new NidData(currBaseNid), Tree.context);
        }catch(final Exception exc){}
        for(int i = 0; i < this.num_components; i++){
            try{
    ...
    public boolean check(final String expressions[], final String[] messages) {
        ...
        for(idx = 0; idx < this.device_components.size(); idx++){
            ...
            if(currId != null && !currId.trim().equals("")){
                ...
                if(currData != null) varExpr.append("_" + currId + " = " + Tree.dataToString(currData) + ";");
    ...
    public void configure(final RemoteTree subtree, final int baseNid) {
        this.configure(subtree, baseNid, null);
    }

    public void configure(final RemoteTree subtree, final int baseNid, final Node node) {
        ...
        String path = null;
        try{
            final NodeInfo info = subtree.getInfo(new NidData(baseNid), 0);
                ...
                this.pop_items[i].addActionListener(new ActionListener(){
                    ...
                    public void actionPerformed(final ActionEvent e) {
                        ...
                        if(JOptionPane.showConfirmDialog(DeviceSetup.this, "Execute " + DeviceSetup.this.methods[j] + "?", "Execute a device method", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION){
                            try{
                                DeviceSetup.this.subtree.doDeviceMethod(new NidData(DeviceSetup.this.baseNid), DeviceSetup.this.methods[j], Tree.context);
                            }catch(final Exception exc){
                                errmsg = exc.toString();
                                success = false;
                            }
    ...
    public void fireUpdate(final String id, final Data val) {
    ...
    void setHighlight(final boolean isHighlighted, final int[] nids) {
    ...
    public void updateIdentifiers() {
        ...
        for(int idx = 0; idx < this.device_components.size(); idx++){
            ...
            if(currId != null){
                final Data currData = (this.device_components.elementAt(idx)).getData();
                if(currData != null) varExpr.append("_" + currId + " = " + Tree.dataToString(currData) + ";");
                if((this.device_components.elementAt(idx)).getState()) varExpr.append("_" + currId + "_state = 1; ");
                else varExpr.append("_" + currId + "_state = 0; ");
            }
        }
        if(this.device_components.size() > 0) Data.evaluate(varExpr.toString());
    }
}
