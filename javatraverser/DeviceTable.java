...
public class DeviceTable extends DeviceComponent{
    ...
    @Override
    public void displayData(final Data data, final boolean is_on) {
        this.state = is_on;
        final String decompiled = Tree.dataToString(data);
    ...
    public Data getData() {
        ...
        if(this.useExpressions) return Tree.dataFromExpr("COMPILE(\'" + DeviceTable.expandBackslash(dataString) + "\')");
        else return Tree.dataFromExpr(dataString);
    }
    ...
    @Override
    public void initializeData(final Data data, final boolean is_on) {
        this.initializing = true;
        String decompiled = "";
        try{
            ...
            }else decompiled = Tree.dataToString(this.subtree.evaluateData(data, 0));
            ...
            */
            ...
            public int getColumnCount() {
                if(DeviceTable.this.refMode == DeviceTable.REFLEX || DeviceTable.this.refMode == DeviceTable.REFLEX_INVERT){
                    ...
                    }else return DeviceTable.this.numCols / 2 + 1;
                }else{
                    if(DeviceTable.this.displayRowNumber || (DeviceTable.this.rowNames != null && DeviceTable.this.rowNames.length > 0)){
                        return DeviceTable.this.numCols + 1;
                    }else return DeviceTable.this.numCols;
                }
            }
            ...
            public String getColumnName(final int idx) {
                try{
                    if(DeviceTable.this.displayRowNumber || (DeviceTable.this.rowNames != null && DeviceTable.this.rowNames.length > 0)){
                        if(idx == 0) return "";
                        else return DeviceTable.this.columnNames[idx - 1];
                    }else return DeviceTable.this.columnNames[idx];
                }catch(final Exception exc){
                    return "";
                }
            }
