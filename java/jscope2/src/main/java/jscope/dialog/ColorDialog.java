...
public final class ColorDialog extends JDialog implements ActionListener, ItemListener{
	...
	public void colorSetItems(final String[] color_name_in, final Color[] colors) {
		for(int i = 0; i < color_name_in.length; i++)
			...
			}else this.color_set.addElement(new Item(color_name_in[i], colors[i]));
	}
	...
	public void fromFile(final Properties pr, final String prompt) {
		...
		if(this.getNumColor() == 0) if(this.main_scope.js_prop != null) this.getPropertiesValue();
	...
	public int insertItemAt(final String name, final Color color_in) {
		int i;
		String ext = "";
		int extIdx = 1;
		for(i = 0; i < this.color_set.size(); i++){
			...
			if(this.color_set.elementAt(i).color.equals(color_in)) return i;
		}
		final Item c_item = new Item(name + ext, color_in);
		this.color_set.insertElementAt(c_item, i);
		return i;
	}

	public void insertItemAt(final String name, final Color color_in, final int idx) {
		this.color_set.insertElementAt(new Item(name, color_in), idx);
	}
	...
	public void keyPressedAction(final KeyEvent e) {
		...
		if(key == KeyEvent.VK_DELETE) if(ob == this.colorList){
			final int idx = this.colorList.getSelectedIndex();
			this.listModel.remove(idx);
			this.color_set.removeElementAt(idx);
			this.colorName.setText("");
		}
		if(key == KeyEvent.VK_ENTER) if(ob == this.colorName) this.addUpdateItem(this.colorName.getText(), this.getColor());
	}
	...
	void setSliderToColor(final Color c) {
	...
	private void addUpdateItem(final String name, final Color color_in) {
		int i;
		if(name == null || name.length() == 0) return;
		final Item c_item = new Item(name, color_in);
	...
	private void reversedColor(final String[] color_name_in, final Color[] colors) {
		for(int i = 0; i < color_name_in.length; i++)
			if(colors[i].equals(this.reversed ? Color.black : Color.white)){
