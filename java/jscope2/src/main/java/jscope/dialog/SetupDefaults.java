...
public final class SetupDefaults extends JDialog implements ActionListener{
	...
	private final JComboBox<String>	grid_mode, legend_mode, auto_color_mode;
	JLabel							lab;
	...
	public SetupDefaults(final Frame fw, final String frame_title, final jScopeDefaultValues def_vals){
		...
		panel1.add(this.lab);
		this.auto_color_mode = new JComboBox<String>();
	...
	public void actionPerformed(final ActionEvent e) {
		...
		if(ob == this.reset) this.initialize();
	}
	...
	public boolean isChanged(final jScopeDefaultValues def_vals_in) {
		if(!jScopeFacade.equalsString(this.shot.getText(), def_vals_in.shot_str)) return true;
		if(!jScopeFacade.equalsString(this.experiment.getText(), def_vals_in.experiment_str)) return true;
		if(!jScopeFacade.equalsString(this.upd_event.getText(), def_vals_in.upd_event_str)) return true;
		if(!jScopeFacade.equalsString(this.def_node.getText(), def_vals_in.def_node_str)) return true;
		if(!jScopeFacade.equalsString(this.title.getText(), def_vals_in.title_str)) return true;
		if(!jScopeFacade.equalsString(this.x_max.getText(), def_vals_in.xmax)) return true;
		if(!jScopeFacade.equalsString(this.x_min.getText(), def_vals_in.xmin)) return true;
		if(!jScopeFacade.equalsString(this.x_label.getText(), def_vals_in.xlabel)) return true;
		if(!jScopeFacade.equalsString(this.y_max.getText(), def_vals_in.ymax)) return true;
		if(!jScopeFacade.equalsString(this.y_min.getText(), def_vals_in.ymin)) return true;
		if(!jScopeFacade.equalsString(this.y_label.getText(), def_vals_in.ylabel)) return true;
		if(this.reversed_b.isSelected() != def_vals_in.reversed) return true;
		if(this.upd_limits.isSelected() != def_vals_in.upd_limits) return true;
		return false;
	}

	public void saveDefaultConfiguration(final jScopeDefaultValues def_vals_in) {
		def_vals_in.experiment_str = new String(this.experiment.getText());
		def_vals_in.shot_str = new String(this.shot.getText());
		def_vals_in.xmax = new String(this.x_max.getText());
		def_vals_in.xmin = new String(this.x_min.getText());
		def_vals_in.ymax = new String(this.y_max.getText());
		def_vals_in.ymin = new String(this.y_min.getText());
		def_vals_in.title_str = new String(this.title.getText());
		def_vals_in.xlabel = new String(this.x_label.getText());
		def_vals_in.ylabel = new String(this.y_label.getText());
		def_vals_in.upd_event_str = new String(this.upd_event.getText());
		def_vals_in.def_node_str = new String(this.def_node.getText());
		def_vals_in.upd_limits = this.upd_limits.isSelected();
		...
		this.y_grid_lines.setText("" + this.y_curr_lines_grid);
		def_vals_in.setIsEvaluated(false);
	}

	public void show(final Frame f, final jScopeDefaultValues def_vals_in) {
		this.def_vals = def_vals_in;
		this.initialize();
		this.pack();
