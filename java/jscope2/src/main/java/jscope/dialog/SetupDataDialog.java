...
import jscope.jScopeMultiWave;
import jscope.jScopeWaveInterface;
...
import jscope.waveform.WaveformListener;

public final class SetupDataDialog extends JDialog implements ActionListener, ItemListener, KeyListener, WaveformListener{
	...
	class Data{
		...
		public void copy(final Data ws) {
			...
			if(ws.xexpr != null) this.xexpr = new String(ws.xexpr);
			if(ws.yexpr != null) this.yexpr = new String(ws.yexpr);
		...
		public boolean equals(final Data ws) {
			if(this.xexpr != null){
				if(!this.xexpr.equals(ws.xexpr)) return false;
			}else if(ws.xexpr != null && ws.xexpr.length() != 0) return false;
			if(this.yexpr != null){
				if(!this.yexpr.equals(ws.yexpr)) return false;
			}else if(ws.yexpr != null && ws.yexpr.length() != 0) return false;
			if(this.label != null){
	...
	class ExpandExp extends JDialog implements ActionListener{
		...
		private final JButton			dok, dcancel;
		private final JTextArea			xexpr, yexpr;
		...
		ExpandExp(final Frame _fw, final SetupDataDialog conf_diag){
			...
			p2.add(this.lab_y);
			p1.add(p2);
			this.yexpr = new JTextArea(50, 20);
			JScrollPane scroller = new JScrollPane(this.yexpr);
			p1.add(scroller);
			final JPanel p3 = new JPanel(new FlowLayout(FlowLayout.LEFT));
			this.lab_x = new JLabel("X Expression:");
			p3.add(this.lab_x);
			p1.add(p3);
			this.xexpr = new JTextArea(50, 20);
			scroller = new JScrollPane(this.xexpr);
			p1.add(scroller);
			final JPanel p = new JPanel();
			p.setLayout(new FlowLayout(FlowLayout.CENTER));
			this.dok = new JButton("Ok");
			this.dok.addActionListener(this);
			p.add(this.dok);
			this.dcancel = new JButton("Cancel");
		...
		public void actionPerformed(final ActionEvent e) {
			...
			if(ob == this.dok){
				this.conf_dialog.x_expr.setText(this.xexpr.getText());
				this.conf_dialog.y_expr.setText(this.yexpr.getText());
		...
		public void setExpressionString(final String x, final String y) {
			...
			if(x != null) this.xexpr.setText(x);
			if(y != null) this.yexpr.setText(y);
		}
	}
	class SError extends JDialog implements ActionListener{
		...
		public void actionPerformed(final ActionEvent e) {
			...
			if(ob == this.dcancel) this.setVisible(false);
		}
	...
	class SList extends JPanel implements ItemListener{
		...
		private final DefaultListModel<String>	list_model			= new DefaultListModel<String>();
		private int								list_num_shot		= 0;
		private JTextField						marker_step_t;
		private JComboBox<Mode1D>				mode1D;
		private JComboBox<Mode2D>				mode2D;
		private JComboBox<String>				color;
		JComboBox<Marker>						marker;
		...
		public SList(){
			...
			p.setLayout(gridbag);
			this.mode1D = new JComboBox<Mode1D>();
			for(Mode1D mode : Mode1D.values())
				this.mode1D.addItem(mode);
			this.mode1D.addItemListener(this);
			gridbag.setConstraints(this.mode1D, c);
			p.add(this.mode1D);
			this.mode2D = new JComboBox<Mode2D>();
			for(Mode2D mode : Mode2D.values())
				this.mode2D.addItem(mode);
			this.mode2D.addItemListener(this);
			gridbag.setConstraints(this.mode2D, c);
			p.add(this.mode2D);
			this.color = new JComboBox<String>();
			this.setColorList();
			this.color.addItemListener(this);
			gridbag.setConstraints(this.color, c);
			p.add(this.color);
			this.marker = new JComboBox<Marker>();
			for(final Marker element : Marker.values())
			...
			this.marker_step_t.addFocusListener(new FocusAdapter(){
				...
				public void focusLost(final FocusEvent e) {
					if(SList.this.getSignalSelect() != -1) try{
		...
		public void addSignals() {
			...
			if(idx == -1){
				if(this.shots != null && this.shots.length != 0) for(int i = 0; i < this.shots.length; i++, ws = this.getSignalSetup()){
					ws.cshot = this.shots[i];
					...
					ws.cshot = SetupDataDialog.UNDEF_SHOT;
					this.addSignalSetup(ws);
					this.signalListAdd(ws);
				}
		...
		public boolean evaluateShotList(String in_shot) throws IOException {
			...
			final int new_shots[] = SetupDataDialog.this.wi.getShotArray(in_shot);
			if(new_shots == null){
			...
			this.shots = new_shots;
			if(SetupDataDialog.this.image_b.isSelected()) if(this.shots.length > 1) this.shots = new int[]{this.shots[0]};
			return true;
		}
		...
		public Data getSignalSetup() {
			...
			ws.xexpr = new String(SetupDataDialog.this.x_expr.getText());
			ws.yexpr = new String(SetupDataDialog.this.y_expr.getText());
			// ws.interpolate = (mode1D.getSelectedIndex() == 0 ? true : false);
			this.setPlotMode1D(ws, (Mode1D)this.mode1D.getSelectedItem());
			this.setPlotMode2D(ws, (Mode2D)this.mode2D.getSelectedItem());
			ws.marker = (Marker)this.marker.getSelectedItem();
			try{
		...
		public void init(final WaveInterface wi_in) {
			if(wi_in != null){
				if(wi_in.shots != null){
					this.shots = new int[wi_in.num_shot];
					for(int i = 0; i < wi_in.num_shot; i++)
						this.shots[i] = wi_in.shots[i];
				}
				this.list_num_shot = wi_in.num_shot;
				for(int i = 0; i < wi_in.num_waves; i++){
					final Data ws = new Data();
					ws.label = wi_in.in_label[i];
					ws.xexpr = wi_in.in_x[i];
					ws.yexpr = wi_in.in_y[i];
					ws.up_err = wi_in.in_up_err[i];
					ws.low_err = wi_in.in_low_err[i];
					ws.interpolate = wi_in.interpolates[i];
					ws.mode2D = wi_in.mode2D[i];
					ws.mode1D = wi_in.mode1D[i];
					ws.marker = wi_in.markers[i];
					ws.marker_step = wi_in.markers_step[i];
					ws.color_idx = wi_in.colors_idx[i];
					if(wi_in.shots != null && wi_in.shots.length > i) ws.cshot = wi_in.shots[i];
					else ws.cshot = SetupDataDialog.UNDEF_SHOT;
					this.addSignalSetup(ws);
				}
				this.signalListRefresh();
				if(this.getSignalSelect() == -1 && wi_in.num_waves > 0) this.setSignalSelect(0);
			}
		...
		public void itemStateChanged(final ItemEvent e) {
			...
			if(ob == this.marker){
				final Marker mark = (Marker)this.marker.getSelectedItem();
				this.signals.elementAt(this.getSignalSelect()).marker = mark;
				this.setMarkerTextState(mark);
			}
			if(ob == this.mode1D) this.setPlotMode1D(this.signals.elementAt(this.getSignalSelect()), (Mode1D)this.mode1D.getSelectedItem());
			if(ob == this.mode2D) this.setPlotMode2D(this.signals.elementAt(this.getSignalSelect()), (Mode2D)this.mode2D.getSelectedItem());
			if(ob == this.color) this.signals.elementAt(this.getSignalSelect()).color_idx = this.color.getSelectedIndex();
		}
		...
		public void putSignalSetup(final Data ws) {
			...
			if(ws.xexpr != null) SetupDataDialog.this.x_expr.setText(ws.xexpr);
			else SetupDataDialog.this.x_expr.setText("");
			if(ws.yexpr != null) SetupDataDialog.this.y_expr.setText(ws.yexpr);
			else SetupDataDialog.this.y_expr.setText("");
			this.mode1D.setSelectedItem(ws.mode1D);
			this.mode2D.setSelectedItem(ws.mode2D);
			this.marker.setSelectedItem(ws.marker);
		...
		public void setColorList() {
			...
			if(colors_name != null) for(final String element : colors_name)
		...
		public void signalsRefresh() {
			...
			for(int j = 0; j < num_sig; j += n_shot){
				for(int i = 0; i < n_shot; i++){
					if(i < this.list_num_shot) this.signals.setElementAt(this.signals.elementAt(j + i), k);
					else{
						final Data ws = new Data();
					...
					if(this.shots != null) this.signals.elementAt(k).cshot = this.shots[i];
					else this.signals.elementAt(k).cshot = SetupDataDialog.UNDEF_SHOT;
					k++;
				}
		...
		public void updateList() throws IOException {
			if(this.getSignalSelect() == -1) SetupDataDialog.this.signalList.addSignals();
			else if(SetupDataDialog.this.y_expr.getText().trim().length() != 0) SetupDataDialog.this.signalList.updateSignals();
			...
			if(this.getSignalSelect() != -1){
				...
				for(i = 0; i < num_signal; i++)
					if(i >= start_idx && i < end_idx){
						...
						this.signals.elementAt(i).xexpr = SetupDataDialog.this.x_expr.getText();
						this.signals.elementAt(i).yexpr = SetupDataDialog.this.y_expr.getText();
		...
		private void resetSignalSetup() {
			...
			this.setMarkerTextState(null);
			this.color.setSelectedIndex(0);
		}

		private void setMarkerTextState(final Marker marker) {
			if(marker == null || marker == Marker.NONE){
				this.marker_step_t.setText("1");
				this.marker_step_t.setEditable(false);
			}else this.marker_step_t.setEditable(true);
		}
		...
		private void setPlotMode1D(final Data ws, final Mode1D mode) {
			ws.interpolate = mode != Mode1D.NOLINE;
			ws.mode1D = mode;
		}

		private void setPlotMode2D(final Data ws, final Mode2D mode) {
			ws.mode2D = mode;
		}

		private void signalListAdd(final Data ws) {
			if(ws.cshot != SetupDataDialog.UNDEF_SHOT){
				if(ws.xexpr == null || ws.xexpr.length() == 0) this.list_model.addElement("Y : " + this.getExpressionList(ws.yexpr) + " Shot : " + ws.cshot);
				else this.list_model.addElement("Y : " + this.getExpressionList(ws.yexpr) + " X : " + this.getExpressionList(ws.xexpr) + " Shot : " + ws.cshot);
			}else if(ws.xexpr == null || ws.xexpr.length() == 0) this.list_model.addElement("Y : " + this.getExpressionList(ws.yexpr) + " Shot : Undef");
			else this.list_model.addElement("Y : " + this.getExpressionList(ws.yexpr) + " X : " + this.getExpressionList(ws.xexpr) + " Shot : Undef");
		}

		private void signalListReplace(final int idx, final Data ws) {
			if(ws.cshot != SetupDataDialog.UNDEF_SHOT){
				if(ws.xexpr == null || ws.xexpr.length() == 0) this.list_model.setElementAt("Y : " + this.getExpressionList(ws.yexpr) + " Shot : " + ws.cshot, idx);
				else this.list_model.setElementAt("Y : " + this.getExpressionList(ws.yexpr) + " X : " + this.getExpressionList(ws.xexpr) + " Shot : " + ws.cshot, idx);
			}else if(ws.xexpr == null || ws.xexpr.length() == 0) this.list_model.setElementAt("Y : " + this.getExpressionList(ws.yexpr) + " Shot : Undef", idx);
			else this.list_model.setElementAt("Y : " + this.getExpressionList(ws.yexpr) + " X : " + this.getExpressionList(ws.xexpr) + " Shot : Undef", idx);
		}
	...
	public void actionPerformed(final ActionEvent e) {
		...
		// this.wave.wi.is_image = this.image_b.isSelected();
		// wave.jScopeErase();
		...
		if(ob == this.error && this.y_expr.getText().trim().length() != 0){
			if(this.signalList.getSignalSelect() == -1)
			    /*
			    Check if the list is in add mode (signal selected -1)
	...
	public boolean isChanged(final Data s[]) {
		...
		for(int i = 0; i < wave_wi.num_waves; i++){
			if(!jScopeFacade.equalsString(s[i].xexpr, wave_wi.in_x[i])) return true;
			if(!jScopeFacade.equalsString(s[i].yexpr, wave_wi.in_y[i])) return true;
	...
	public void keyPressed(final KeyEvent e) {
		...
		if(key == KeyEvent.VK_ENTER) if(ob == this.y_expr || ob == this.x_expr || ob == this.shot || ob == this.experiment || ob == this.signal_label){
			this.updateDataSetup();
			e.consume();
		}
		if(ob instanceof TextField) if(ob == this.x_max || ob == this.y_max || ob == this.x_min || ob == this.y_min || ob == this.shot || ob == this.time_max || ob == this.time_min) if(!Character.isDigit(key) && key != KeyEvent.VK_DELETE && key != '.' && key != '+' && key != '-') return;
	}

	@Override
	public void keyReleased(final KeyEvent e) {/*stub*/}

	@Override
	public void keyTyped(final KeyEvent e) {/*stub*/}
	...
	public void processWaveformEvent(final WaveformEvent e) {
		...
		switch(e.getID()){
			case WaveformEvent.END_UPDATE:
				...
				if(full_error != null) JOptionPane.showMessageDialog(this, full_error, "alert", JOptionPane.ERROR_MESSAGE);
	...
	public void putWindowSetup(final jScopeWaveInterface wi_in) {
		if(wi_in == null){
			this.eraseForm();
			return;
		}
		if(wi_in.is_image) wi_in.defaults = wi_in.defaults & ~((1 << jScopeWaveInterface.B_y_max) + (1 << jScopeWaveInterface.B_y_min));
		this.setImageDialog(wi_in.is_image);
		// this.wi.evaluated_shot = wi.evaluated_shot;
		this.wi.colorProfile = wi_in.colorProfile;
		this.wi.cexperiment = wi_in.cexperiment;
		this.wi.cin_shot = wi_in.cin_shot;
		this.wi.cin_upd_event = wi_in.cin_upd_event;
		this.wi.in_upd_event = wi_in.in_upd_event;
		this.wi.last_upd_event = wi_in.last_upd_event;
		this.wi.cin_def_node = wi_in.cin_def_node;
		this.wi.cin_xmax = wi_in.cin_xmax;
		this.wi.cin_xmin = wi_in.cin_xmin;
		this.wi.cin_ymax = wi_in.cin_ymax;
		this.wi.cin_ymin = wi_in.cin_ymin;
		this.wi.cin_upd_limits = wi_in.cin_upd_limits;
		this.wi.cin_timemin = wi_in.cin_timemin;
		this.wi.cin_timemax = wi_in.cin_timemax;
		this.wi.cin_title = wi_in.cin_title;
		this.wi.cin_xlabel = wi_in.cin_xlabel;
		this.wi.cin_ylabel = wi_in.cin_ylabel;
		this.wi.legend_x = wi_in.legend_x;
		this.wi.legend_y = wi_in.legend_y;
		this.wi.show_legend = wi_in.show_legend;
		this.wi.reversed = wi_in.reversed;
		this.wi.is_image = wi_in.is_image;
		this.wi.keep_ratio = wi_in.keep_ratio;
		this.wi.horizontal_flip = wi_in.horizontal_flip;
		this.wi.vertical_flip = wi_in.vertical_flip;
		this.image_b.setSelected(wi_in.is_image);
		this.keep_ratio_b.setSelected(wi_in.keep_ratio);
		this.horizontal_flip_b.setSelected(wi_in.horizontal_flip);
		this.vertical_flip_b.setSelected(wi_in.vertical_flip);
		this.setDefaultFlags(wi_in.defaults);
		...
		this.x_expr.setText("");
		this.y_expr.setText("");
		this.x_log.setSelected(wi_in.x_log);
		this.y_log.setSelected(wi_in.y_log);
		// GAB 2014
		this.continuous_update_b.setSelected(wi_in.isContinuousUpdate);
	...
	private int checkSetup() {
		...
		if(def_exp ^ def_shot) if(!def_shot){
	...
	private void setImageDialog(final boolean state) {
		if(state){
			...
			// Continues update is experimental and therefore not shown in production interface
			this.continuous_update_b.setVisible(false);
			this.pix_y_max.setVisible(false);
			this.pix_y_min.setVisible(false);
	...
	private int updateWI() {
		...
		this.wi.markers = new Marker[num_signal];
		this.wi.markers_step = new int[num_signal];
		this.wi.colors_idx = new int[num_signal];
		this.wi.interpolates = new boolean[num_signal];
		this.wi.mode2D = new Mode2D[num_signal];
		this.wi.mode1D = new Mode1D[num_signal];
		// GAB 2014
		// wi.isContinuousUpdate = continuous_update_b.isSelected();
		this.wi.isContinuousUpdate = false;
		if(s[0].cshot != SetupDataDialog.UNDEF_SHOT) this.wi.shots = new int[num_signal];
		for(int i = 0; i < num_signal; i++){
			if(s[i].label != null) this.wi.in_label[i] = new String(s[i].label);
			if(s[i].xexpr != null) this.wi.in_x[i] = new String(s[i].xexpr);
			if(s[i].yexpr != null) this.wi.in_y[i] = new String(s[i].yexpr);
			if(this.wi.shots != null) this.wi.shots[i] = s[i].cshot;
			if(!this.wi.is_image){
