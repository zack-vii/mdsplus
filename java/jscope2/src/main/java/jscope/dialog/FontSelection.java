...
public class FontSelection extends JDialog implements ActionListener, ItemListener{
	...
	public FontSelection(final Frame dw, final String title){
		...
		this.fonts = new JComboBox<Object>(this.envfonts);
	...
	public void actionPerformed(final ActionEvent e) {
		...
		if(ob == this.ok || ob == this.apply){
			if(this.waveform_i.isSelected()){
				this.font = this.getFontChoice();
				this.main_scope.updateFont();
				this.main_scope.repaintAllWaves();
				this.main_scope.setChange(true);
			}
			if(this.application_i.isSelected()) this.main_scope.setApplicationFonts(this.getFontChoice());
	...
	public void fromFile(final Properties pr, final String prompt) {
		String prop;
		if((prop = pr.getProperty(prompt)) != null) this.font = this.stringToFont(prop);
		if(this.font != null){
			this.setFontChoice();
	...
	public void itemStateChanged(final ItemEvent e) {
		...
		this.fontC.changeFont(this.getFontChoice());
		this.repaint();
	}
	...
	public Font stringToFont(final String f) {
		...
		if(DEBUG.D) System.out.println("Font desiderato " + f);
		...
		Font lfont = new Font(this.fontchoice, this.stChoice, Integer.parseInt(this.siChoice));
		// Check if selected font can be displayed
		// on the current platform
		if(!this.fontC.isFontAvailable(lfont)){
			// Set font to default font
			this.fontchoice = "Default";
			this.stChoice = 0;
			this.siChoice = "12";
			lfont = new Font(this.fontchoice, this.stChoice, Integer.parseInt(this.siChoice));
		}
		if(DEBUG.D) System.out.println("Font: " + lfont);
		return lfont;
	}
	...
	private void getPropertiesValue() {
		...
		if((prop = js_prop.getProperty("jScope.font.application")) != null) this.main_scope.setApplicationFonts(this.stringToFont(prop));
...
class FontPanel extends JPanel{
	...
	public void changeFont(final Font _font) {
		this.font = _font;
		this.repaint();
	}
	...
	// this platform
	public boolean isFontAvailable(final Font _font) {
		int height;
		final Graphics g = this.getGraphics();
		final FontMetrics fm = g.getFontMetrics(_font);
		height = fm.getHeight();
