...
final public class ColorMap{
	...
	private static final class colorMapBuilder{
		...
		public final ColorMap build(final int length_in) {
			final Color[] colors = new Color[this.length];
			final int[] r = ColorMap.getValues(length_in, this.pR, this.vR);
			final int[] g = ColorMap.getValues(length_in, this.pG, this.vG);
			final int[] b = ColorMap.getValues(length_in, this.pB, this.vB);
		...
		public byte[] getBlueIntValues() {
			...
			final int[] c = ColorMap.getValues(this.length, this.pB, this.vB);
		...
		public byte[] getGreenIntValues() {
			final byte b[] = new byte[this.length];
			final int[] c = ColorMap.getValues(this.length, this.pG, this.vG);
			for(int i = 0; i < this.length; i++)
				b[i] = (byte)c[i];
			return b;
		}
		...
		public byte[] getRedIntValues() {
			final byte b[] = new byte[this.length];
			final int[] c = ColorMap.getValues(this.length, this.pR, this.vR);
			for(int i = 0; i < this.length; i++)
				b[i] = (byte)c[i];
			return b;
		}
	...
	public Color getColor(final float val, final float min_in, final float max_in) {
		final int idx = (int)((val - min_in) / (max_in - min_in) * (this.colors.length - 1));
		return this.colors[idx];
	}
