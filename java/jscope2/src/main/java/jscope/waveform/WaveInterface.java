...
public class WaveInterface{
	...
	public static final void freeCache() {/*stub*/}

	static public int[] getShotArray(final String in_shots, final String exp, final DataProvider dp) throws IOException {
		...
		if(shot_list == null || shot_list.length == 0 || shot_list.length > WaveInterface.MAX_NUM_SHOT){
			...
			else if(dp.errorString() != null) error = dp.errorString();
	...
	public static void setBriefError(final boolean state) {
		WaveInterface.brief_error = state;
	}
	...
	static String processShotExpression(String shotExpr, final String exp) {
		...
		while((idx = shotExpr.indexOf('0', prevIdx)) != -1){
			if((idx > 0 && Character.isLetterOrDigit(shotExpr.charAt(idx - 1))) || (idx < (shotExpr.length() - 1) && Character.isLetterOrDigit(shotExpr.charAt(idx + 1)))) outStr += shotExpr.substring(prevIdx, idx + 1);
			else outStr += shotExpr.substring(prevIdx, idx) + "current_shot(\"" + exp + "\")";
			prevIdx = idx + 1;
		}
	...
	public ColorProfile		colorProfile		= new ColorProfile();
	public int				colors_idx[];
	private String			curr_error;
	public DataProvider		dp;
	public String			error;
	private int				num_points			= 2048;
	/*
	...
	public Signal.Marker[]	markers;
	public int[]			markers_step;
	public Signal.Mode1D[]	mode1D;
	public Signal.Mode2D[]	mode2D;
	...
	private double			xmax				= Double.POSITIVE_INFINITY, ymax = Double.POSITIVE_INFINITY, timemax = Double.POSITIVE_INFINITY;
	private double			xmin				= Double.NEGATIVE_INFINITY, ymin = Double.NEGATIVE_INFINITY, timemin = Double.NEGATIVE_INFINITY;
	...
	public WaveInterface(final DataProvider dp, final int num_points){
		this(null, dp, num_points);
	}
	...
	public WaveInterface(final Waveform wave, final DataProvider dp, final int num_points){
	...
	public void addFrames(final String frames_in) {
		this.addFrames(frames_in, null);
	}

	public void addFrames(final String frames_in, final String frames_time) {
		if(DEBUG.M) System.out.println("WaveInterface.AddFrames(\"" + frames_in + "\", \"" + frames_time + "\")");
		this.setAsImage(true);
		this.in_x = new String[]{frames_time};
		this.in_y = new String[]{frames_in};
		this.in_low_err = this.in_up_err = this.in_label = new String[]{null};
		this.interpolates = new boolean[]{false};
		this.mode1D = new Signal.Mode1D[]{Signal.Mode1D.LINE};
		this.mode2D = new Signal.Mode2D[]{Signal.Mode2D.IMAGE};
		this.markers = new Signal.Marker[]{Signal.Marker.NONE};
		this.markers_step = this.colors_idx = new int[]{0};
	}
	...
	public boolean addSignals(final String x_expr[], final String y_expr[]) {
		...
		final Signal.Marker[] new_markers = new Signal.Marker[new_num_waves];
		...
		final Signal.Mode2D[] new_mode2D = new Signal.Mode2D[new_num_waves];
		final Signal.Mode1D[] new_mode1D = new Signal.Mode1D[new_num_waves];
		final int[] new_shots = (this.shots == null) ? null : new int[new_num_waves];
	...
	public void enableCache(final boolean state) {/*not implemented*/}
	...
	public synchronized void evaluateOthers() throws Exception {
		...
		for(curr_wave = 0; curr_wave < this.num_waves; curr_wave++)
			if(!this.evaluated[curr_wave] && !(!this.interpolates[curr_wave] && this.markers[curr_wave] == Signal.Marker.NONE)){
	...
	public void evaluateShot(final long shot, final WaveformManager wfm) throws Exception {
		...
		for(curr_wave = 0; curr_wave < this.num_waves; curr_wave++){
			...
			if((shot == 0) || (this.shots[curr_wave] == shot && !this.evaluated[curr_wave] && (this.interpolates[curr_wave] || this.markers[curr_wave] != Signal.Marker.NONE))){
				this.signals[curr_wave] = this.getSignal(curr_wave, this.xmin, this.xmax);
				if(this.signals[curr_wave] == null){
					this.w_error[curr_wave] = this.curr_error;
					this.evaluated[curr_wave] = false;
	...
	public String getErrorTitle(final boolean brief) {
		...
		if(this.num_waves == 0 || (this.is_image && this.frames == null)){
			if(this.error != null) if(brief) er = WaveInterface.getFirstLine(new String(this.error));
			return er;
		}
		for(int ii = 0; ii < this.num_waves; ii++)
			if(this.w_error != null && this.w_error[ii] != null) n_error++;
		if(this.error == null && n_error > 1 && n_error == this.num_waves) er = "Evaluation error on all signals";
		else if(this.error != null) er = this.error;
		else if(n_error == 1 && this.num_waves == 1) er = this.w_error[0];
	...
	public int getSelectedSignal() {
		return this.signal_select;
	}

	public int[] getShotArray(final String in_shots) throws IOException {
		final int[] curr_shots = WaveInterface.getShotArray(in_shots, this.experiment, this.dp);
		return curr_shots;
	}
	...
	public boolean[] getSignalsState() {
		...
		if(this.num_waves != 0){
			...
			for(int i = 0, j = 0; i < this.num_waves; i += ns)
				state[j++] = (this.interpolates[i] || (this.markers[i] != Signal.Marker.NONE));
		}
		return state;
	}
	...
	public boolean getSignalState(final int i) {
		...
		if(i < this.num_waves) state = (this.interpolates[i] || (this.markers[i] != Signal.Marker.NONE));
		return state;
	}
	...
	public void setLimits() {
		...
		for(final Signal signal : this.signals)
			if(signal != null) try{
				signal.setNumPoints(this.num_points);
	...
	public void setSelectedSignal(final int signal_select) {
		this.signal_select = signal_select;
	}

	public void setShotArray(final String in_shot) throws IOException {
		this.in_shot = in_shot;
		final int curr_shots[] = this.getShotArray(in_shot);
			...
			for(int i = 0; i < this.num_waves; i++)
				if(name.equals(this.in_y[i]) || name.equals(this.in_label[i])){
					for(int j = i; j < i + ns; j++){
						...
						this.markers[i] = Signal.Marker.NONE;
					}
					return;
				}
		}
	...
	public boolean updateShot(final int curr_shots[]) {
		...
		this.modified = true;
		final String[] new_in_label = new String[num_signal];
		final String[] new_in_x = new String[num_signal];
		final String[] new_in_y = new String[num_signal];
		final String[] new_in_up_err = new String[num_signal];
		final String[] new_in_low_err = new String[num_signal];
		final Signal.Marker[] new_markers = new Signal.Marker[num_signal];
		final int[] new_markers_step = new int[num_signal];
		final int[] new_colors_idx = new int[num_signal];
		final boolean[] new_interpolates = new boolean[num_signal];
		final Signal.Mode2D[] new_mode2D = new Signal.Mode2D[num_signal];
		final Signal.Mode1D[] new_mode1D = new Signal.Mode1D[num_signal];
		final IntBuffer new_shots;
		if(curr_shots == null) new_shots = null;
		else new_shots = IntBuffer.allocate(num_signal);
		final int sig_idx = (this.num_shot == 0) ? 1 : this.num_shot;
		for(int i = 0, k = 0; i < num_expr; i++){
			if(new_shots != null) new_shots.put(curr_shots);
			for(int j = 0; j < curr_num_shot; j++, k++){
				new_in_label[k] = this.in_label[i * sig_idx];
				new_in_x[k] = this.in_x[i * sig_idx];
				new_in_y[k] = this.in_y[i * sig_idx];
				if(j < this.num_shot){
					new_markers[k] = this.markers[i * this.num_shot + j];
					new_markers_step[k] = this.markers_step[i * this.num_shot + j];
					new_interpolates[k] = this.interpolates[i * this.num_shot + j];
					new_mode2D[k] = this.mode2D[i * this.num_shot + j];
					new_mode1D[k] = this.mode1D[i * this.num_shot + j];
					new_in_up_err[k] = this.in_up_err[i * this.num_shot + j];
					new_in_low_err[k] = this.in_low_err[i * this.num_shot + j];
					new_colors_idx[k] = this.colors_idx[i * this.num_shot + j];
				}else{
					new_markers[k] = this.markers[i * this.num_shot];
					new_markers_step[k] = this.markers_step[i * this.num_shot];
					new_interpolates[k] = this.interpolates[i * this.num_shot];
					new_mode2D[k] = this.mode2D[i * this.num_shot];
					new_mode1D[k] = this.mode1D[i * this.num_shot];
					new_in_up_err[k] = this.in_up_err[i * this.num_shot];
					new_in_low_err[k] = this.in_low_err[i * this.num_shot];
					if(WaveInterface.auto_color_on_expr) new_colors_idx[k] = i % Waveform.colors.length; // this.colors_idx[i * this.num_shot];
					else new_colors_idx[k] = j % Waveform.colors.length;
				}
			}
		}
		...
		this.mode2D = new_mode2D;
		this.mode1D = new_mode1D;
		this.shots = new_shots == null ? null : new_shots.array();
		this.num_shot = new_shots == null ? curr_num_shot : 1;
	...
	private void createNewFramesClass(final int image_type) throws IOException {
		...
		if(image_type == FrameData.JAI_IMAGE) try{
	...
	private Signal getSignal(final int curr_wave, double xmin_in, double xmax_in) {
		if(DEBUG.M) System.out.println("WaveInterface.getSignal(" + curr_wave + ", " + xmin_in + ", " + xmax_in + ")");
		...
		try{
			out_signal = this.getSignalFromProvider(curr_wave, xmin_in, xmax_in);
		}catch(final IOException e){
			e.printStackTrace();
		}
		if(out_signal != null){
			if(this.xmin > xmin_in) xmin_in = this.xmin;
			if(this.xmax < xmax_in) xmax_in = this.xmax;
			out_signal.setXLimits(xmin_in, xmax_in, Signal.AT_CREATION | Signal.FIXED_LIMIT);
	...
	private Signal getSignalFromProvider(final int curr_wave, final double xmin_in, final double xmax_in) throws IOException {
		if(DEBUG.M) System.out.println("WaveInterface.getSignalFromProvider(" + curr_wave + ", " + xmin_in + ", " + xmax_in + ")");
		WaveData up_err = null, low_err = null;
		WaveData wd = null;
		final WaveData xwd = null;
		int yDimension = 1;
		Signal out_signal;
		String new_xlabel = null, new_ylabel = null;
		String new_zlabel = null;
		...
		if(this.in_def_node != null && this.in_def_node.length() > 0) this.dp.setDefault(this.in_def_node);
		else this.dp.setDefault(new StringBuilder("\\").append(this.experiment).append("::TOP").toString());
		if(this.dp.errorString() != null){
			this.error = this.dp.errorString();
			return null;
		}
		...
		else wd = this.dp.getWaveData(this.in_y[curr_wave]);
		if(wd != null){
			...
			new_xlabel = wd.getXLabel();
			new_ylabel = wd.getYLabel();
			new_title = wd.getTitle();
			if(yDimension == 2) new_zlabel = wd.getZLabel();
			if(yDimension == 1){
