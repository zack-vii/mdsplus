...
final public class WaveformEvent extends AWTEvent{
	...
	int				signal_idx;
	public int		start_pixel_x;
	public int		start_pixel_y;
	String			status_info;
	public double	time_value		= Float.NaN;
	public float	values_line[]	= null;
	float			values_signal[];
	public float	values_x[];
	public float	values_y[];
	public int		x_pixel;
	...
	public String toString() {
		...
		switch(event_id){
			case WaveformEvent.MEASURE_UPDATE:
				...
				if(this.showXasDate) s = WaveformEvent.setStrSize(String.format("[%s, %s; dx %s; dy %s]", //
				...
				else s = WaveformEvent.setStrSize(String.format("[%s, %s; dx %s; dy %s; 1/dx %s]", //
			...
			case WaveformEvent.POINT_IMAGE_UPDATE:
				if(s == null) if(!w.isImage()){
					...
					if(xt_string == null) s = WaveformEvent.setStrSize(String.format("[%s, %f]", x_string, new Double(this.point_y)), string_size);
					else s = WaveformEvent.setStrSize(String.format("[%s, %f %s]", x_string, new Double(this.point_y), xt_string), string_size + 40);
				}else if(this.frame_type == FrameData.BITMAP_IMAGE_32 || this.frame_type == FrameData.BITMAP_IMAGE_16) s = WaveformEvent.setStrSize(String.format("[%d, %d : (%f) : %f]", new Integer((int)this.point_x), new Integer((int)this.point_y), new Float(this.point_value), new Float(this.delta_x)), 50);
				else s = WaveformEvent.setStrSize(String.format("[%d, %d : (%06x) : %f]", new Integer((int)this.point_x), new Integer((int)this.point_y), new Integer(this.pixel_value & 0xffffff), new Float(this.delta_x)), 50);
				break;
		}
		return(s == null ? "" : s);
	}
