 ...
 *    protected int rows[] = {2, 2, 0,0,0,0,0,0};
 *
 *    //Resize button class
 ...
 *
 *         //The number of component to add to panel are 4 (2 column each column 2 component)
 *         //Before add component we must add num_component - 1 canvas object that are used
 ...
 *                   {
 *   //Action used to draw resize line on the component, only if component implement double buffering
...
final public class RowColumnLayout implements LayoutManager{
	...
	private static void drawXResize(final Component c, final int x) {
		...
		// return;
		// else
		if(!(c instanceof Waveform)) return;
	 ...
	 * where i-th came form 1 to n_lines. Multi horizontal line must be draw when more
	 ...
	 *            column must be 1.
	 * @param pw
	 *            Vector of normalize width of the column. The sum of pw[x] must be 1
	 */
	...
	public void drawResize(final Component _b, final int x, final int y) {
		...
		int pos;
		Rectangle r;
		int i, j, jj, num_line = 0, y_pos, n_draw, curr_height, start_pos;
		if(this.init_resize){
			this.init_resize = false;
			found = false;
			for(j = 0; j < this.row.length && !found; j++)
		...
		if(this.resize_mode == RowColumnLayout.VERTICAL){
			...
			if(y < 0){
				...
				for(j = 0, jj = 0, i = this.b_comp + this.comp_idx; i > this.b_comp + k - 1; i--, j++){
					...
					if(n_draw > 0){
						y_pos = (r.height + pos) - (-curr_height + r.height + (jj - n_draw) * this.MIN_SIZE_H);
						// ((RowColumnComponent)c).DrawYResize(c, pos_y - 2, n_draw, -MIN_SIZE_H);
						RowColumnLayout.drawYResize(c, y_pos - 2, n_draw, -this.MIN_SIZE_H);
				...
				for(j = 0, jj = 0, i = this.b_comp + this.comp_idx + 1; i < this.b_comp + k; i++, j++){
					...
					if(n_draw > 0){
						y_pos = pos - curr_height + r.height + (jj - n_draw) * this.MIN_SIZE_H;
	...
	public void layoutContainer(final Container parent) {
		...
		for(int i = 0; i < this.row.length; i++)
			if(this.row[i] > 0) this.resizeColumn(parent, i);
	}
	...
	/**
	 * Set component to a columns all with the same height, component portion
	 * of the parent width
	 *
	 * @param col
	...
	private void resetHeight(final int col) {
		if(this.row != null && col < this.row.length){
			...
			for(int i = 0; i < col; i++)
				k += this.row[i];
			if(this.percent_height != null && k + this.row[col] <= this.percent_height.length) for(int j = 0; j < this.row[col]; j++){
	...
	private void resetWidth() {
		if(this.row != null && this.percent_width != null && this.percent_width.length >= this.row.length) for(int i = 0; i < this.column; i++){
	 ...
	 *            column index
	 */
	private void resizeColumn(final Container parent, final int col_idx_in) {
		int k = 0, y = 0;
		int previousHeight = 0, currMaxHeight;
		for(int i = 0; i < col_idx_in; i++)
			k += this.row[i];
		currMaxHeight = this.maxHeight - (this.row[col_idx_in] - 1) * this.vgap;
		for(int j = 0; j < this.row[col_idx_in]; j++){
			...
			if(c.isVisible()){
				...
				if(j != this.row[col_idx_in] - 1) d.height = (int)(currMaxHeight * this.percent_height[k]);
				...
				if(k < this.b_comp){
					...
					if(j == this.row[col_idx_in] - 1) b.setBounds(c.getBounds().x + c.getBounds().width - this.hgap - 2, this.maxHeight - this.bx_pos, 4, 8);
					else{
						int p = this.by_pos;
	...
	private void setSizes(final Container parent) {
		...
		for(int i = 0; i < this.row.length; i++){// column; i++) {
			...
			for(int j = 0; j < this.row[i]; j++){
				final Component c = parent.getComponent(this.b_comp + k);
				if(c.isVisible()) this.percent_height[k] = (float)1. / this.row[i];
				k++;
			}
			this.percent_width[i] = (float)1. / this.column;
		}
