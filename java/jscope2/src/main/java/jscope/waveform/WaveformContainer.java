...
public class WaveformContainer extends RowColumnContainer implements WaveformManager, WaveformListener, Printable{
	...
	final static public void main(final String... args) {
		...
		final WaveformContainer wc = new WaveformContainer(new int[]{1}, true);
		wc.setPopupMenu(new MultiWavePopup());
		wc.setPreferredSize(new Dimension(400, 300));
		wnd.add(wc);
		final MultiWaveform mw = wc.getWavePanel(0);
		mw.setTitle("test sine wave");
		final double x[] = new double[1024];
		final float y[] = new float[1024];
		for(int i = 0; i < x.length; i++){
			x[i] = i / 100.;
			y[i] = (float)Math.sin(x[i]);
		}
		final Signal sig = new Signal(x, y);
		mw.addSignal(sig);
		wnd.pack();
		wnd.setVisible(true);
		mw.autoscale();
	 ...
	 */
	public void addWaveContainerListener(final WaveContainerListener l) {
		if(l == null) return;
		synchronized(this.wave_container_listener){
	...
	public Waveform getSelectPanel() {
		...
		if(this.sel_wave == null){
			int i;
			for(i = 0; i < this.getGridComponentCount(); i++)
				if(this.getWavePanel(i) == null || (this.getWavePanel(i).getShowSignalCount() == 0 && !this.getWavePanel(i).isImage())) break;
			if(i == this.getGridComponentCount()){
				final Component c[] = this.createWaveComponents(1);
				this.splitContainer(c[0]);
				w = (Waveform)c[0];
			}else w = this.getWavePanel(i);
	...
	public int getWaveIndex(final Waveform w) {
		...
		for(idx = 0; idx < this.getGridComponentCount(); idx++)
			if(this.getWavePanel(idx) == w) break;
		if(idx < this.getGridComponentCount()) return idx;
		return -1;
	}

	public MultiWaveform getWavePanel(final int idx) {
		final Component c = this.getGridComponent(idx);
		if(c instanceof MultiWaveform) return (MultiWaveform)c;
		return null;
	}
	...
	public void loadFileConfiguration() {/*stub*/}
	...
	public int print(final Graphics g, final PageFormat pf, final int pageIndex) throws PrinterException {
		...
		if(pageIndex == 0){
			...
			if(ver.indexOf("1.2") != -1) if(pf.getOrientation() == PageFormat.LANDSCAPE){
				st_y = -13;
				st_x = 15;
				width -= 5;
			}else{
		...
		switch(e.getID()){
			...
			case WaveformEvent.MEASURE_UPDATE:
				if(w.getMode() == Waveform.MODE_POINT){
					...
					this.updateCrosshairs(x, y, (Waveform)e.getSource());
				}
				/*
	 ...
	 *            the waveform container listener
	 */
	public void removeContainerListener(final ActionListener l) {
		if(l == null) return;
		synchronized(this.wave_container_listener){
	 ...
	 * @see MultiWaveform
	 */
	public void updateCrosshairs(final double x, final double y, final Waveform curr_w) {
		Waveform w;
		for(int i = 0; i < this.getGridComponentCount(); i++){
			w = this.getWavePanel(i);
			if(w != null && w != curr_w) w.updateCrosshair(x, y);
		}
	}

	@Override
	public void updateCrosshairs(final double x, final Waveform curr_w) {
		this.updateCrosshairs(x, Double.NaN, curr_w);
	}
	...
	protected MultiWaveform createWaveComponent() {
		final MultiWaveform wave = new MultiWaveform();
	 ...
	 */
	protected MultiWaveform[] createWaveComponents(final int num) {
		final MultiWaveform mws[] = new MultiWaveform[num];
		for(int i = 0; i < mws.length; i++)
			mws[i] = this.createWaveComponent();
		return mws;
	}
	...
	protected void dispatchWaveContainerEvent(final WaveContainerEvent e) {
		final WaveContainerListener[] list;
		synchronized(this.wave_container_listener){
			list = new WaveContainerListener[this.wave_container_listener.size()];
			this.wave_container_listener.copyInto(list);
		}
		for(final WaveContainerListener wcl : list)
			wcl.processWaveContainerEvent(e);
	}
