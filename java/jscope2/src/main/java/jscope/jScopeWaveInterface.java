...
public final class jScopeWaveInterface extends WaveInterface{
	...
	static public String containMainShot(final String in_shot, final String m_shot) {
		...
		int idx;
		String out = in_shot;
		if((in_shot != null) && (idx = in_shot.indexOf("#")) != -1) if(m_shot != null) out = "[" + in_shot.substring(0, idx) + m_shot + in_shot.substring(idx + 1) + "]";
	...
	public jScopeWaveInterface(final jScopeWaveInterface wi){
		...
		this.markers = new Marker[this.num_waves];
		...
		this.mode2D = new Mode2D[this.num_waves];
		this.mode1D = new Mode1D[this.num_waves];
		...
		else this.shots = new int[this.num_waves];
	...
	public jScopeWaveInterface(final Waveform wave, final DataProvider dp, final jScopeDefaultValues def_vals, final boolean enable_cache){
		this(wave, dp, def_vals, enable_cache, 2048);
	}

	public jScopeWaveInterface(final Waveform wave, final DataProvider dp, final jScopeDefaultValues def_vals, final boolean enable_cache, final int num_points){
		super(dp, num_points);
	...
	public void addEvent(final UpdateEventListener w) throws IOException {
		...
		if(this.in_upd_event == null || !this.in_upd_event.equals(new_event)) this.addEvent(w, new_event);
	}
	...
	public void createVector() {
		...
		this.in_label = new String[this.num_waves];
		this.shots = new int[this.num_waves];
		this.in_y = new String[this.num_waves];
		this.in_x = new String[this.num_waves];
		this.in_up_err = new String[this.num_waves];
		this.in_low_err = new String[this.num_waves];
	...
	public void fromFile(final Properties pr, final String prompt, final ColorMapDialog cmd) throws IOException {
		...
		try{
			...
			if(prop != null){
				...
				if(y_str.indexOf("[") == 0 && y_str.indexOf("$roprand") != -1) try{
				...
				if(num_expr > 1) try{
					...
					while(st_y.hasMoreTokens()){
						...
						if(this.in_y[i].indexOf("multitrace") != -1) this.in_y[i] = "compile" + this.in_y[i].substring(this.in_y[i].indexOf("("));
						i++;
					}
				}catch(final Exception e){
					num_expr = 1;
				...
				if(num_expr == 1){
					this.in_y[0] = y_str;
					if(this.in_y[0].indexOf("multitrace") != -1) this.in_y[0] = "compile" + y_str.substring(y_str.indexOf("("));
			...
			for(int idx = 1; idx <= num_expr; idx++){
				...
				for(int s = 1; s <= this.num_shot; s++){
					...
					this.mode1D[expr_idx + s] = Mode1D.fromString(prop);
					prop = pr.getProperty(prompt + ".mode_2D_" + idx + "_" + s);
					this.mode2D[expr_idx + s] = Mode2D.fromString(prop);
		...
		if(!this.is_image){
			...
			for(; i < this.w_error.length; i++){
				...
				if(e != null){
					...
					}else if(WaveInterface.brief_error) full_error = full_error + e.substring(0, idx) + "\n";
	...
	public void toFile(final PrintWriter out, final String prompt) {
		 ...
		 */
		jScopeWaveInterface.writeLine(out, prompt + "x_label: ", this.cin_xlabel);
		jScopeWaveInterface.writeLine(out, prompt + "y_label: ", this.cin_ylabel);
		if(!this.is_image){
			jScopeWaveInterface.writeLine(out, prompt + "x_log: ", "" + this.x_log);
			jScopeWaveInterface.writeLine(out, prompt + "y_log: ", "" + this.y_log);
			jScopeWaveInterface.writeLine(out, prompt + "update_limits: ", "" + this.cin_upd_limits);
			if(this.show_legend) jScopeWaveInterface.writeLine(out, prompt + "legend: ", "(" + this.legend_x + "," + this.legend_y + ")");
		}else{
			jScopeWaveInterface.writeLine(out, prompt + "is_image: ", "" + this.is_image);
			jScopeWaveInterface.writeLine(out, prompt + "keep_ratio: ", "" + this.keep_ratio);
			jScopeWaveInterface.writeLine(out, prompt + "horizontal_flip: ", "" + this.horizontal_flip);
			jScopeWaveInterface.writeLine(out, prompt + "vertical_flip: ", "" + this.vertical_flip);
		}
		if(this.colorProfile != null) if(this.colorProfile.useRGB) jScopeWaveInterface.writeLine(out, prompt + "useRGB: ", "" + this.colorProfile.useRGB);
		else{
			jScopeWaveInterface.writeLine(out, prompt + "palette: ", "" + this.colorProfile.colorMap.name);
			jScopeWaveInterface.writeLine(out, prompt + "bitShift: ", "" + this.colorProfile.bitShift);
			jScopeWaveInterface.writeLine(out, prompt + "bitClip: ", "" + this.colorProfile.bitClip);
		}
		jScopeWaveInterface.writeLine(out, prompt + "experiment: ", this.cexperiment);
		jScopeWaveInterface.writeLine(out, prompt + "event: ", this.cin_upd_event);
		jScopeWaveInterface.writeLine(out, prompt + "default_node: ", this.cin_def_node);
		jScopeWaveInterface.writeLine(out, prompt + "num_shot: ", "" + cnum_shot);
		if(cnum_shot != 0){
			if(this.useDefaultShot()) eval_shot = this.num_shot > 0 ? this.num_shot : 1;
			else eval_shot = cnum_shot;
			jScopeWaveInterface.writeLine(out, prompt + "num_expr: ", "" + this.num_waves / eval_shot);
		}else{
			jScopeWaveInterface.writeLine(out, prompt + "num_expr: ", "" + this.num_waves);
			// Shot is not defined in the pannel.
			// cnum_shot must be set to 1 to save, in the configuration file,
			// signal view parameters
			cnum_shot = 1;
		}
		jScopeWaveInterface.writeLine(out, prompt + "shot: ", this.cin_shot);
		jScopeWaveInterface.writeLine(out, prompt + "ymin: ", this.cin_ymin);
		jScopeWaveInterface.writeLine(out, prompt + "ymax: ", this.cin_ymax);
		jScopeWaveInterface.writeLine(out, prompt + "xmin: ", this.cin_xmin);
		jScopeWaveInterface.writeLine(out, prompt + "xmax: ", this.cin_xmax);
		if(this.is_image){
			jScopeWaveInterface.writeLine(out, prompt + "time_min: ", this.cin_timemin);
			jScopeWaveInterface.writeLine(out, prompt + "time_max: ", this.cin_timemax);
		}
		// GAB 2014
		jScopeWaveInterface.writeLine(out, prompt + "continuous_update: ", this.isContinuousUpdate ? "1" : "0");
		// ///////
		jScopeWaveInterface.writeLine(out, prompt + "title: ", this.cin_title);
		jScopeWaveInterface.writeLine(out, prompt + "global_defaults: ", "" + this.defaults);
		for(exp = 0, exp_n = 1; exp < this.num_waves; exp += eval_shot, exp_n++){
			jScopeWaveInterface.writeLine(out, prompt + "label" + "_" + exp_n + ": ", this.in_label[exp]);
			// add blank at the end of expression to fix bug when last expression character is \
			jScopeWaveInterface.writeLine(out, prompt + "x_expr" + "_" + exp_n + ": ", WaveInterface.addNewLineCode(this.in_x[exp]));
			jScopeWaveInterface.writeLine(out, prompt + "y_expr" + "_" + exp_n + ": ", WaveInterface.addNewLineCode(this.in_y[exp]));
			if(!this.is_image){
				jScopeWaveInterface.writeLine(out, prompt + "up_error" + "_" + exp_n + ": ", this.in_up_err[exp]);
				jScopeWaveInterface.writeLine(out, prompt + "low_error" + "_" + exp_n + ": ", this.in_low_err[exp]);
				for(sht = 0, sht_n = 1; sht < cnum_shot; sht++, sht_n++){
					jScopeWaveInterface.writeLine(out, prompt + "mode_1D" + "_" + exp_n + "_" + sht_n + ": ", "" + this.mode1D[exp + sht]);
					jScopeWaveInterface.writeLine(out, prompt + "mode_2D" + "_" + exp_n + "_" + sht_n + ": ", "" + this.mode2D[exp + sht]);
					jScopeWaveInterface.writeLine(out, prompt + "color" + "_" + exp_n + "_" + sht_n + ": ", "" + this.colors_idx[exp + sht]);
					jScopeWaveInterface.writeLine(out, prompt + "marker" + "_" + exp_n + "_" + sht_n + ": ", "" + this.markers[exp + sht]);
					jScopeWaveInterface.writeLine(out, prompt + "step_marker" + "_" + exp_n + "_" + sht_n + ": ", "" + this.markers_step[exp + sht]);
				}
			}
		}
	}
	...
	public String update() throws IOException {
		...
		try{
			this.updateShot();
			if(this.error == null) this.updateDefault();
			else this.signals = null;
	...
	public void updateShot() throws IOException {
		...
		if(this.useDefaultShot()){
			...
			}else if(this.def_vals.getIsEvaluated() && this.def_vals.shots != null) curr_shots = this.def_vals.shots;
			else{
		...
		}else curr_shots = this.getShotArray(jScopeWaveInterface.containMainShot(this.cin_shot, main_shot_str));
