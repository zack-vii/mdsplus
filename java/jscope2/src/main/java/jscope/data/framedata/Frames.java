...
public final class Frames extends Canvas{
	...
	static final class FrameCache{
		...
		private final FrameDescriptor getFrameDescr(final int idx) {
			...
			try{
				...
				exc.printStackTrace();
				System.err.println("# >> Error Loading frame at " + idx + ": " + exc.toString());
				return null;
			}
		}
		...
		private final void loadFrame(final int idx) throws Exception {
			...
			switch(this.frameType){
				...
				case FrameData.AWT_IMAGE:
					...
					this.pixelSize = 0; // pixelSize == 0 means that the image is not a bitmap (jpg, gif...)
	...
	Vector<Double>				frame_time		= new Vector<Double>();
	...
	*/
	...
	public final float getFrameTime() {
		...
		float t_out = 0;
		if(this.curr_frame_idx != -1 && this.frame_time.size() != 0) t_out = this.frame_time.elementAt(this.curr_frame_idx).floatValue();
		return t_out;
	}
	...
	public final void setFrameData(final FrameData fd) throws Exception {
		...
		for(final double element : t)
			this.frame_time.addElement(Double.valueOf(element));
	}
	...
	public final void setZoomRegion(final int idx, final Dimension d, final Rectangle r) {
		...
		}else dim = new Dimension(this.zoom_rect.width, this.zoom_rect.height);
	...
	*/
	...
	private final Point getImageBufferPoint(final int x, final int y) {
		...
		p.x = x;
		p.y = y;
		if(this.horizontal_flip) p.x = this.img_width - x - 1;
		if(this.vertical_flip) p.y = this.img_height - y - 1;
		return p;
	}
	...
	private final int[] getPixelArray(final int idx, final int x, final int y, int img_w, int img_h) {
		...
		final int pixels[] = new int[img_w * img_h];
		final PixelGrabber grabber = new PixelGrabber(img, x, y, img_w, img_h, pixels, 0, img_w);
		try{
			grabber.grabPixels();
