...
public class ContourSignal{
	private class EdgeCaseException extends Exception{
		private static final long serialVersionUID = 1L;

		public EdgeCaseException(final String msg){
			super(msg);
		}
	}
			...
			for(int j = 0; j < this.y.length - 1; j++){
				...
				while(!contourCompleted){
					do{
						try{
							switch(edgeCase){
								...
								case CASE_D:
									...
									break;
								default:
									throw new EdgeCaseException("invalid edgeCase: " + edgeCase);
							}
							int rri = 0;
							int rrj = 0;
							for(l = 0; l < 3; l++){
						...
						}catch(final EdgeCaseException exc){
							if(DEBUG.D) System.err.println("EdgeCaseException");
							/*
