...
final public class XYData{
	private final static boolean checkIncreasingX(final double[] x) {
		for(int i = 1; i < x.length; i++)
			if(x[i] < x[i - 1]) return false;
		return true;
	}

	private final static boolean checkIncreasingX(final long[] xLong) {
		for(int i = 1; i < xLong.length; i++)
			if(xLong[i] < xLong[i - 1]) return false;
		return true;
	}
	...
	private static final double[] long2double(final long[] xLong) {
		...
		for(int i = 0; i < xLong.length; i++)
			x[i] = xLong[i] / 1e9; // assume ns and convert to s;
		return x;
	}
	...
	public XYData(final double[] x, final float[] y, final double resolution){
		this(x, null, y, resolution, XYData.checkIncreasingX(x));
	}
	...
	public XYData(final long[] xLong, final float[] y, final double resolution){
		this(null, xLong, y, resolution, XYData.checkIncreasingX(xLong));
	}
	...
	public XYData(final XYData old, final XYData add){
		if(old == null || !old.increasingX || !add.increasingX){
			...
			for(samplesBefore = 0; samplesBefore < old.x.length; samplesBefore++)
				if(old.x[samplesBefore] >= add.getXMin()) break;
			if(samplesBefore > 0 && samplesBefore < old.x.length && old.x[samplesBefore] > add.getXMin()) samplesBefore--;
			for(samplesAfter = 0; samplesAfter < old.x.length - 1; samplesAfter++)
				if(old.x[old.x.length - samplesAfter - 1] <= add.getX()[add.getX().length - 1]) break;
	...
	private XYData(final double[] x, final long[] xLong, final float[] y, final double resolution, boolean increasingX){
		...
		else this.x = XYData.long2double(xLong);
		this.xLong = xLong;
		this.y = y;
		this.resolution = resolution;
		this.nSamples = (this.x.length < this.y.length) ? this.x.length : this.y.length;
		if(this.isEmpty()){
